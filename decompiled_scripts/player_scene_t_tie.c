...
BOOL func_2() // Position - 0x108
{
	...
	num = SYSTEM::VDIST2(ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true), uLocal_82);
...
int func_6(int iParam0, Entity eParam1) // Position - 0x217
{
	...
	if (!STREAMING::IS_PLAYER_SWITCH_IN_PROGRESS())
	{
		...
		else if (eParam1 == func_7(Global_100402) && eParam1 != 0)
		{
			Global_100402 = 0;
		}
	}
...
void func_20() // Position - 0x54F
{
	...
	while (!flag && num < 400)
	{
		...
		for (i = 0; i < iLocal_77; i = i + 1)
		{
			if (iLocal_77[i] != 0)
			{
				STREAMING::REQUEST_MODEL(iLocal_77[i]);
			
				if (!STREAMING::HAS_MODEL_LOADED(iLocal_77[i]))
				{
					STREAMING::REQUEST_MODEL(iLocal_77[i]);
					flag = false;
				}
		...
		if (!MISC::IS_STRING_NULL_OR_EMPTY(&uLocal_117))
		{
			STREAMING::REQUEST_ANIM_DICT(&uLocal_117);
		
			if (!STREAMING::HAS_ANIM_DICT_LOADED(&uLocal_117))
			{
				STREAMING::REQUEST_ANIM_DICT(&uLocal_117);
				flag = false;
			}
	...
	for (i = 0; i < iLocal_72; i = i + 1)
	{
		if (iLocal_77[i] != 0)
		{
			iLocal_72[i] = PED::CREATE_PED(PED_TYPE_MISSION, iLocal_77[i], uLocal_82 + uLocal_86[i /*3*/], 0, true, true);
			ENTITY::SET_ENTITY_ROTATION(iLocal_72[i], { fLocal_85, 0f, 0f } + uLocal_99[i /*3*/], 2, true);
			PED::SET_PED_RANDOM_COMPONENT_VARIATION(iLocal_72[i], 0);
		
			if (iLocal_70 == 288)
			{
				PED::SET_PED_COMPONENT_VARIATION(iLocal_72[i], PV_COMP_UPPR, 1, 0, 0);
				PED::SET_PED_COMPONENT_VARIATION(iLocal_72[i], PV_COMP_LOWR, 0, 0, 0);
			}
			else
			{
				PED::SET_PED_RANDOM_COMPONENT_VARIATION(iLocal_72[i], 0);
			...
			PED::SET_PED_CAN_BE_TARGETTED(iLocal_72[i], false);
			TASK::TASK_PLAY_ANIM_ADVANCED(iLocal_72[i], &uLocal_117, &uLocal_125[i /*8*/], uLocal_82 + uLocal_86[i /*3*/], { fLocal_85, 0f, 0f } + uLocal_99[i /*3*/], 1000f, -8f, -1, iLocal_158, 0, 2, 0);
			PED::FORCE_PED_AI_AND_ANIMATION_UPDATE(iLocal_72[i], false, false);
			PED::SET_PED_KEEP_TASK(iLocal_72[i], true);
			PED::SET_PED_RESET_FLAG(iLocal_72[i], 55, true);
	...
	for (i = 0; i < iLocal_159; i = i + 1)
	{
		if (uLocal_166[i] > 0)
		{
			...
			opacity = 1f;
			timeout = -1f;
			iLocal_159[i] = GRAPHICS::ADD_DECAL(uLocal_166[i], uLocal_82 + uLocal_173[i /*3*/], unk, unk4, width, height, rCoef, gCoef, bCoef, opacity, timeout, false, false, false);
			SYSTEM::WAIT(0);
		}
	}
...
void func_22() // Position - 0x916
{
	...
	switch (iLocal_70)
	{
		case 280:
			...
			uLocal_173[4 /*3*/] = { ((uLocal_173[0 /*3*/] + uLocal_173[2 /*3*/]) / { 2f, 2f, 2f }) + { 0f, MISC::GET_RANDOM_FLOAT_IN_RANGE(0.5f, 0.5f), MISC::GET_RANDOM_FLOAT_IN_RANGE(0.5f, 0.5f) } };
			uLocal_173[5 /*3*/] = { ((uLocal_173[1 /*3*/] + uLocal_173[2 /*3*/]) / { 2f, 2f, 2f }) + { 0f, MISC::GET_RANDOM_FLOAT_IN_RANGE(0.5f, 0.5f), MISC::GET_RANDOM_FLOAT_IN_RANGE(0.5f, 0.5f) } };
			break;
...
void func_34() // Position - 0x91FB
{
	...
	{
		if (iLocal_77[i] != 0)
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(iLocal_77[i]);
	}

	if (!MISC::IS_STRING_NULL_OR_EMPTY(&uLocal_117))
		STREAMING::REMOVE_ANIM_DICT(&uLocal_117);

	for (i = 0; i < iLocal_159; i = i + 1)
	{
		GRAPHICS::IS_DECAL_ALIVE(iLocal_159[i]);
	}
