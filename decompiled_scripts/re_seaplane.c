#region Local Var
	int iLocal_0 = 0;
	var uLocal_1 = 0;
	var uLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	int iLocal_12 = 0;
	var uLocal_13 = 0;
	var uLocal_14 = 0;
	float fLocal_15 = 0f;
	var uLocal_16 = 0;
	var uLocal_17 = 0;
	int iLocal_18 = 0;
	var uLocal_19 = 0;
	var uLocal_20 = 0;
	char* sLocal_21 = 0;
	float fLocal_22 = 0f;
	var uLocal_23 = 0;
	var uLocal_24 = 0;
	var uLocal_25 = 0;
	float fLocal_26 = 0f;
	float fLocal_27 = 0f;
	var uLocal_28 = 0;
	var uLocal_29 = 0;
	var uLocal_30 = 0;
	float fLocal_31 = 0f;
	float fLocal_32 = 0f;
	float fLocal_33 = 0f;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	int iLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	int iLocal_47 = 0;
	int iLocal_48 = 0;
	var uLocal_49 = 11;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	var uLocal_62 = 0;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	var uLocal_72 = 0;
	var uLocal_73 = 0;
	var uLocal_74 = 0;
	var uLocal_75 = 0;
	var uLocal_76 = 0;
	var uLocal_77 = 0;
	var uLocal_78 = 0;
	var uLocal_79 = 0;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 0;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 5;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	char* sLocal_137 = 0;
	char* sLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	int iLocal_148 = 0;
	int iLocal_149 = 0;
	var uLocal_150 = 0;
	int iLocal_151 = 0;
	int iLocal_152 = 0;
	int iLocal_153 = 0;
	int iLocal_154 = 0;
	int iLocal_155 = 0;
	int iLocal_156 = 0;
	int iLocal_157 = 0;
	var uLocal_158 = 2;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 0;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 1;
	var uLocal_173 = 0;
	var uLocal_174 = 16;
	var uLocal_175 = 0;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	var uLocal_186 = 0;
	var uLocal_187 = 0;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	var uLocal_198 = 0;
	var uLocal_199 = 0;
	var uLocal_200 = 0;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 0;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	var uLocal_210 = 0;
	var uLocal_211 = 0;
	var uLocal_212 = 0;
	var uLocal_213 = 0;
	var uLocal_214 = 0;
	var uLocal_215 = 0;
	var uLocal_216 = 0;
	var uLocal_217 = 0;
	var uLocal_218 = 0;
	var uLocal_219 = 0;
	var uLocal_220 = 0;
	var uLocal_221 = 0;
	var uLocal_222 = 0;
	var uLocal_223 = 0;
	var uLocal_224 = 0;
	var uLocal_225 = 0;
	var uLocal_226 = 0;
	var uLocal_227 = 0;
	var uLocal_228 = 0;
	var uLocal_229 = 0;
	var uLocal_230 = 0;
	var uLocal_231 = 0;
	var uLocal_232 = 0;
	var uLocal_233 = 0;
	var uLocal_234 = 0;
	var uLocal_235 = 0;
	var uLocal_236 = 0;
	var uLocal_237 = 0;
	var uLocal_238 = 0;
	var uLocal_239 = 0;
	var uLocal_240 = 0;
	var uLocal_241 = 0;
	var uLocal_242 = 0;
	var uLocal_243 = 0;
	var uLocal_244 = 0;
	var uLocal_245 = 0;
	var uLocal_246 = 0;
	var uLocal_247 = 0;
	var uLocal_248 = 0;
	var uLocal_249 = 0;
	var uLocal_250 = 0;
	var uLocal_251 = 0;
	var uLocal_252 = 0;
	var uLocal_253 = 0;
	var uLocal_254 = 0;
	var uLocal_255 = 0;
	var uLocal_256 = 0;
	var uLocal_257 = 0;
	var uLocal_258 = 0;
	var uLocal_259 = 0;
	var uLocal_260 = 0;
	var uLocal_261 = 0;
	var uLocal_262 = 0;
	var uLocal_263 = 0;
	var uLocal_264 = 0;
	var uLocal_265 = 0;
	var uLocal_266 = 0;
	var uLocal_267 = 0;
	var uLocal_268 = 0;
	var uLocal_269 = 0;
	var uLocal_270 = 0;
	var uLocal_271 = 0;
	var uLocal_272 = 0;
	var uLocal_273 = 0;
	var uLocal_274 = 0;
	var uLocal_275 = 0;
	var uLocal_276 = 0;
	var uLocal_277 = 0;
	var uLocal_278 = 0;
	var uLocal_279 = 0;
	var uLocal_280 = 0;
	var uLocal_281 = 0;
	var uLocal_282 = 0;
	var uLocal_283 = 0;
	var uLocal_284 = 0;
	var uLocal_285 = 0;
	var uLocal_286 = 0;
	var uLocal_287 = 0;
	var uLocal_288 = 0;
	var uLocal_289 = 0;
	var uLocal_290 = 0;
	var uLocal_291 = 0;
	var uLocal_292 = 0;
	var uLocal_293 = 0;
	var uLocal_294 = 0;
	var uLocal_295 = 0;
	var uLocal_296 = 0;
	var uLocal_297 = 0;
	var uLocal_298 = 0;
	var uLocal_299 = 0;
	var uLocal_300 = 0;
	var uLocal_301 = 0;
	var uLocal_302 = 0;
	var uLocal_303 = 0;
	var uLocal_304 = 0;
	var uLocal_305 = 0;
	var uLocal_306 = 0;
	var uLocal_307 = 0;
	var uLocal_308 = 0;
	var uLocal_309 = 0;
	var uLocal_310 = 0;
	var uLocal_311 = 0;
	var uLocal_312 = 0;
	var uLocal_313 = 0;
	var uLocal_314 = 0;
	var uLocal_315 = 0;
	var uLocal_316 = 0;
	var uLocal_317 = 0;
	var uLocal_318 = 0;
	var uLocal_319 = 0;
	var uLocal_320 = 0;
	var uLocal_321 = 0;
	var uLocal_322 = 0;
	var uLocal_323 = 0;
	var uLocal_324 = 0;
	var uLocal_325 = 0;
	var uLocal_326 = 0;
	var uLocal_327 = 0;
	var uLocal_328 = 0;
	var uLocal_329 = 0;
	var uLocal_330 = 0;
	var uLocal_331 = 0;
	var uLocal_332 = 0;
	var uLocal_333 = 0;
	var uLocal_334 = 0;
	var uLocal_335 = 0;
	var uLocal_336 = 0;
	var uLocal_337 = 0;
	var uLocal_338 = 0;
	var uLocal_339 = 0;
	var uScriptParam_0 = 0;
	var uScriptParam_1 = 5;
	var uScriptParam_2 = 0;
	var uScriptParam_3 = 0;
	var uScriptParam_4 = 0;
	var uScriptParam_5 = 0;
	var uScriptParam_6 = 0;
	var uScriptParam_7 = 0;
	var uScriptParam_8 = 0;
	var uScriptParam_9 = 0;
	var uScriptParam_10 = 0;
	var uScriptParam_11 = 0;
	var uScriptParam_12 = 0;
	var uScriptParam_13 = 0;
	var uScriptParam_14 = 0;
	var uScriptParam_15 = 0;
	var uScriptParam_16 = 0;
	var uScriptParam_17 = 5;
	var uScriptParam_18 = 0;
	var uScriptParam_19 = 0;
	var uScriptParam_20 = 0;
	var uScriptParam_21 = 0;
	var uScriptParam_22 = 0;
#endregion

void main() // Position - 0x0
{
	iLocal_0 = 3;
	iLocal_3 = 1;
	iLocal_4 = 134;
	iLocal_5 = 134;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 1;
	iLocal_9 = 134;
	iLocal_10 = 1;
	iLocal_11 = 12;
	iLocal_12 = 12;
	fLocal_15 = 0.001f;
	iLocal_18 = -1;
	sLocal_21 = "NULL";
	fLocal_22 = 0f;
	fLocal_26 = -0.0375f;
	fLocal_27 = 0.17f;
	fLocal_31 = 80f;
	fLocal_32 = 140f;
	fLocal_33 = 180f;
	iLocal_39 = 1;
	iLocal_40 = 65;
	iLocal_41 = 49;
	iLocal_42 = 64;
	sLocal_137 = "RE_SeaPlane1";
	sLocal_138 = "RE_SeaPlane2";
	uLocal_139 = { 0f, 0f, 0f };
	uLocal_145 = { 3097.46f, 2181.55f, 0.5f };
	iLocal_151 = -1;
	iLocal_152 = -1;
	iLocal_153 = joaat("dodo");
	iLocal_154 = joaat("tropic");
	iLocal_155 = joaat("G_M_Y_MexGoon_02");
	iLocal_156 = joaat("suntrap");
	iLocal_157 = joaat("G_M_Y_ArmGoon_02");
	uLocal_142 = { uScriptParam_0.f_1[0 /*3*/] };

	if (unk_0x55EEDBBFDC6E810F(11))
	{
		if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]))
			unk_0x439314AD4FC79186(uLocal_116[0 /*4*/], 2);
	
		func_169(true, false);
	}

	if (func_168() && func_125(uLocal_142, 32, 0, false, true))
		func_122(32);
	else
		unk_0x675D9C12C73D3DE7();

	while (true)
	{
		if (unk_0xCBE2EC2868A6C438() || func_121(1) || func_121(4))
		{
			if (func_121(7))
				func_82();
		
			switch (iLocal_47)
			{
				case 0:
					func_35();
					break;
			
				case 1:
					func_1();
					break;
			}
		}
		else
		{
			if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]))
				unk_0x439314AD4FC79186(uLocal_116[0 /*4*/], 2);
		
			func_169(true, false);
		}
	
		SYSTEM::WAIT(0);
	}

	return;
}

void func_1() // Position - 0x18B
{
	switch (iLocal_48)
	{
		case 0:
			iLocal_48 = 1;
			break;
	
		case 1:
			switch (iLocal_148)
			{
				case 0:
					iLocal_48 = 2;
					break;
			}
			break;
	
		case 2:
			func_2();
			break;
	}

	return;
}

void func_2() // Position - 0x1CF
{
	func_32(66, true);
	func_32(67, true);
	func_6(-1, 0);
	func_3();
	func_169(true, false);
	return;
}

void func_3() // Position - 0x1F5
{
	func_4();
	return;
}

int func_4() // Position - 0x202
{
	if (_IS_MISSION_REPEAT_ACTIVE(false))
		return 0;

	if (Global_100720.f_8)
		if (Global_100720.f_10 > 0)
			return 0;
	else if (Global_100720.f_10 > 1)
		return 0;

	Global_100720.f_10 = Global_100720.f_10 + 1;
	return 1;
}

BOOL _IS_MISSION_REPEAT_ACTIVE(BOOL bExcludeBenchmark) // Position - 0x24D
{
	if (!bExcludeBenchmark && unk_0x24B651D85CCE5EB4(joaat("benchmark")) > 0)
		return true;

	return IS_BIT_SET(Global_78807, 0);
}

void func_6(int iParam0, int iParam1) // Position - 0x275
{
	if (iParam0 == -1)
		iParam0 = func_30();

	if (iParam0 == -1)
		return;

	if (iParam1 <= func_29(iParam0))
	{
		func_28(iParam0, iParam1);
	
		if (!func_27(51))
		{
			func_17("RE_REWARD" /*Some Random Events will reward the player with stat boosts or money.*/, 1, 0, 4000, 10000, func_20(), 0, 138, 0);
			func_16(51);
		}
	
		if (func_15(iParam0))
			Global_113648.f_24997.f_2 = 3;
	
		if (func_14(iParam0, iParam1) != 322)
			func_8(func_14(iParam0, iParam1), uLocal_43, uLocal_43.f_1);
	
		Global_113636 = iParam1;
	
		if (Global_113634 == 0)
			if (Global_113637 == 1 || Global_113637 == 5 || Global_113637 == 11 || Global_113637 == 25)
				func_7(2);
			else if (Global_113637 == 26 || Global_113637 == 8 || Global_113637 == 17)
				func_7(7);
			else
				func_7(1);
	}

	return;
}

void func_7(int iParam0) // Position - 0x378
{
	Global_113634 = iParam0;
	return;
}

void func_8(int iParam0, var uParam1, var uParam2) // Position - 0x386
{
	BOOL flag;

	iParam0 < 0;

	if (iParam0 == 321 || iParam0 > 321)
	{
	}
	else
	{
		_STAT_SET_PACKED_BOOL(891 + iParam0, 1, -1);
	}

	flag = true;

	if (Global_113648.f_10196[iParam0 /*12*/].f_5 == 1)
	{
		if (Global_113648.f_10196[iParam0 /*12*/].f_6 == 11 || Global_113648.f_10196[iParam0 /*12*/].f_6 == 12)
			flag = false;
	}
	else
	{
		Global_113648.f_10196[iParam0 /*12*/].f_5 = 1;
		Global_113648.f_10196[iParam0 /*12*/].f_10 = uParam1;
		Global_113648.f_10196[iParam0 /*12*/].f_11 = uParam2;
	
		if (iParam0 == 287)
			unk_0xE571C8D0AF67E3B4(joaat("NUM_HIDDEN_PACKAGES_0"), 50, 0);
	
		if (iParam0 == 286)
			unk_0xE571C8D0AF67E3B4(joaat("NUM_HIDDEN_PACKAGES_1"), 50, 0);
	
		if (iParam0 == 299)
			unk_0xE571C8D0AF67E3B4(joaat("NUM_HIDDEN_PACKAGES_3"), 50, 0);
	}

	if (flag)
		func_9();

	return;
}

void func_9() // Position - 0x46C
{
	int num;
	float num2;
	float num3;
	float num4;
	float num5;
	float num6;
	float num7;
	float num8;
	float value;
	int num9;

	num = 0;
	Global_113384 = 0;
	Global_113385 = 0;
	Global_113386 = 0;
	Global_113387 = 0;
	Global_113388 = 0;
	Global_113389 = 0;
	Global_113390 = 0;
	num2 = 0f;
	num3 = 0f;
	num4 = 0f;
	num5 = 0f;
	num6 = 0f;
	num7 = 0f;
	num8 = 0f;
	value = Global_113648.f_10196.f_3853;
	Global_113648.f_10196.f_3853 = 0f;

	while (num < 321)
	{
		if (Global_113648.f_10196[num /*12*/].f_5 == 1)
		{
			switch (Global_113648.f_10196[num /*12*/].f_6)
			{
				case 1:
					Global_113384 = Global_113384 + 1;
					num2 = num2 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				case 3:
					Global_113385 = Global_113385 + 1;
					num3 = num3 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				case 5:
					Global_113386 = Global_113386 + 1;
					num4 = num4 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				case 7:
					Global_113387 = Global_113387 + 1;
					num5 = num5 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				case 9:
					Global_113388 = Global_113388 + 1;
					num6 = num6 + (Global_113648.f_10196[num /*12*/].f_4 * 4f);
					break;
			
				case 11:
					Global_113389 = Global_113389 + 1;
					num7 = num7 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				case 13:
					Global_113390 = Global_113390 + 1;
					num8 = num8 + Global_113648.f_10196[num /*12*/].f_4;
					break;
			
				default:
					break;
			}
		}
	
		num = num + 1;
	}

	if (Global_113367 > 0)
		if (Global_113384 == Global_113367)
			num2 = 55f;

	if (Global_113368 > 0)
		if (Global_113385 == Global_113368)
			num3 = 10f;

	if (Global_113369 > 0)
		if (Global_113386 == Global_113369)
			num4 = 0f;

	if (Global_113370 > 0)
		if (Global_113387 == Global_113370)
			num5 = 10f;

	if (Global_113371 > 0)
	{
		if (Global_113388 == Global_113371 || (Global_113371 * 10) / Global_113388 < 41 || Global_113388 > Global_113374 || Global_113388 == Global_113374)
		{
			if (!IS_BIT_SET(Global_113648.f_10196.f_3856, 14))
			{
				if (Global_113388 == Global_113371)
				{
					unk_0xE571C8D0AF67E3B4(joaat("NUM_RNDEVENTS_COMPLETED"), Global_113371, 0);
					unk_0xECDAB41968FF21A8(&(Global_113648.f_10196.f_3856), 14);
				}
			}
		
			num6 = 5f;
		}
	}

	if (Global_113372 > 0)
		if (Global_113389 == Global_113372)
			num7 = 15f;

	if (Global_113373 > 0)
		if (Global_113390 == Global_113373)
			num8 = 5f;

	Global_113648.f_10196.f_3853 = num2 + num3 + num4 + num5 + num6 + num7 + num8;

	if (Global_113388 > Global_113374 || Global_113388 == Global_113374)
		num9 = Global_113374;
	else
		num9 = Global_113388;

	unk_0x3CC35ACFFC9C730E(joaat("NUM_MISSIONS_COMPLETED"), Global_113384, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_MISSIONS_AVAILABLE"), Global_113367, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_MINIGAMES_COMPLETED"), Global_113385, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_MINIGAMES_AVAILABLE"), Global_113368, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_ODDJOBS_COMPLETED"), Global_113386, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_ODDJOBS_AVAILABLE"), Global_113369, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_RNDPEOPLE_COMPLETED"), Global_113387, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_RNDPEOPLE_AVAILABLE"), Global_113370, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_RNDEVENTS_COMPLETED"), num9, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_RNDEVENTS_AVAILABLE"), Global_113374, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_MISC_COMPLETED"), Global_113390 + Global_113389, 1);
	unk_0x3CC35ACFFC9C730E(joaat("NUM_MISC_AVAILABLE"), Global_113373 + Global_113372, 1);
	Global_113391 = (Global_113384 * 100) / Global_113367;
	Global_113393 = ((Global_113386 + Global_113385) * 100) / (Global_113369 + Global_113368);
	Global_113392 = ((Global_113387 + num9) * 100) / (Global_113370 + Global_113374);
	Global_113394 = ((Global_113389 + Global_113390) * 100) / (Global_113372 + Global_113373);
	unk_0x683F4BAF21D6EE25(joaat("TOTAL_PROGRESS_MADE"), Global_113648.f_10196.f_3853, 1);
	unk_0x3CC35ACFFC9C730E(joaat("PERCENT_STORY_MISSIONS"), Global_113391, 1);
	unk_0x3CC35ACFFC9C730E(joaat("PERCENT_AMBIENT_MISSIONS"), Global_113392, 1);
	unk_0x3CC35ACFFC9C730E(joaat("PERCENT_ODDJOBS"), Global_113393, 1);

	if (value > 0f && SYSTEM::FLOOR(value) < SYSTEM::FLOOR(Global_113648.f_10196.f_3853))
		func_11(13, SYSTEM::FLOOR(Global_113648.f_10196.f_3853));

	if (!unk_0xBAB6476CDD8C4AA6())
	{
		if (!Global_78558)
		{
			if (func_10() == 2 == false && !unk_0xA26A9A07F761D8F8())
			{
				if (unk_0x3FFDF8D1413D95DE())
					Global_113382 = 0;
			
				if (!Global_63362)
					func_4();
			}
		}
	}

	return;
}

int func_10() // Position - 0x92A
{
	return Global_32163;
}

int func_11(int iParam0, int iParam1) // Position - 0x935
{
	int num;

	if (iParam0 < 0)
		return 0;

	if (iParam0 > 78)
		return 0;

	if (iParam1 <= 0 || iParam1 > 100)
		return 0;

	num = unk_0xED220A261CE0D89C(iParam0);

	if (iParam1 > num)
		return unk_0x0B3E167D2C1443C7(iParam0, iParam1);

	return 0;
}

void _STAT_SET_PACKED_BOOL(int iParam0, int iParam1, int iParam2) // Position - 0x986
{
	if (iParam2 == -1)
		iParam2 = func_13();

	unk_0x0111091C0EE35B9C(iParam0, iParam1, iParam2);
	return;
}

int func_13() // Position - 0x9A4
{
	return Global_1574918;
}

int func_14(int iParam0, int iParam1) // Position - 0x9B0
{
	switch (iParam0)
	{
		case 0:
			return 250;
	
		case 1:
			return 226;
	
		case 2:
			return 243;
	
		case 3:
			return 256;
	
		case 4:
			return 259;
	
		case 5:
			if (iParam1 == 1)
				return 281;
		
			if (iParam1 == 2)
				return 282;
			break;
	
		case 6:
			return 265;
	
		case 7:
			return 218;
	
		case 9:
			if (iParam1 == 1)
				return 271;
		
			if (iParam1 == 2)
				return 272;
		
			if (iParam1 == 3)
				return 273;
		
			if (iParam1 == 4)
				return 274;
		
			if (iParam1 == 5)
				return 275;
		
			if (iParam1 == 6)
				return 276;
		
			if (iParam1 == 7)
				return 277;
		
			if (iParam1 == 8)
				return 278;
		
			if (iParam1 == 9)
				return 279;
		
			if (iParam1 == 10)
				return 280;
			break;
	
		case 10:
			return 219;
	
		case 11:
			if (iParam1 == 1)
				return 246;
		
			if (iParam1 == 2)
				return 247;
		
			if (iParam1 == 3)
				return 248;
		
			if (iParam1 == 4)
				return 249;
			break;
	
		case 12:
			return 254;
	
		case 13:
			if (iParam1 == 1)
				return 260;
		
			if (iParam1 == 2)
				return 261;
		
			if (iParam1 == 3)
				return 262;
		
			if (iParam1 == 4)
				return 264;
			break;
	
		case 14:
			return 283;
	
		case 15:
			if (iParam1 == 1)
				return 224;
		
			if (iParam1 == 2)
				return 225;
			break;
	
		case 16:
			return 252;
	
		case 17:
			if (iParam1 == 1)
				return 244;
		
			if (iParam1 == 2)
				return 245;
			break;
	
		case 18:
			return 253;
	
		case 19:
			return 215;
	
		case 20:
			return 216;
	
		case 21:
			return 251;
	
		case 22:
			if (iParam1 == 1)
				return 221;
		
			if (iParam1 == 2)
				return 222;
			break;
	
		case 23:
			if (iParam1 == 1)
				return 213;
		
			if (iParam1 == 2)
				return 214;
			break;
	
		case 24:
			return 242;
	
		case 25:
			if (iParam1 == 1)
				return 267;
		
			if (iParam1 == 2)
				return 268;
		
			if (iParam1 == 3)
				return 269;
			break;
	
		case 8:
			return 255;
	
		case 26:
			if (iParam1 == 1)
				return 227;
		
			if (iParam1 == 2)
				return 228;
			break;
	
		case 27:
			if (iParam1 == 1)
				return 257;
		
			if (iParam1 == 2)
				return 258;
			break;
	
		case 28:
			return 217;
	
		case 29:
			if (iParam1 == 1)
				return 229;
		
			if (iParam1 == 2)
				return 230;
		
			if (iParam1 == 3)
				return 231;
			break;
	
		case 30:
			return 285;
	
		case 31:
			return 318;
	
		case 32:
			return 319;
	
		case 33:
			return 320;
	}

	return 322;
}

BOOL func_15(int iParam0) // Position - 0xD24
{
	switch (iParam0)
	{
		case 29:
		case 30:
		case 2:
		case 18:
			return false;
	}

	return true;
}

void func_16(int iParam0) // Position - 0xD53
{
	int num;
	int i;

	num = iParam0;

	for (i = 0; num > 31; i = i + 1)
	{
		num = num - 32;
	}

	if (i < 3)
		unk_0xECDAB41968FF21A8(&Global_113648.f_20412.f_150[i], num);

	return;
}

void func_17(char* sParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7, int iParam8) // Position - 0xD95
{
	func_18(sParam0, "", iParam1, iParam2, iParam3, iParam4, iParam5, iParam6, iParam7, iParam8);
	return;
}

void func_18(char* sParam0, char* sParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7, int iParam8, int iParam9) // Position - 0xDB6
{
	int i;

	if (unk_0x4310A0DB886F9FED(sParam0, ""))
		return;

	if (iParam3 < 0)
		return;

	if (iParam5 < 500 && iParam5 != -1)
		return;

	if (iParam4 < 0 && iParam4 != -1)
		return;

	if (iParam6 < 1 || iParam6 > 7)
		return;

	if (iParam7 == 235)
		return;

	if (iParam8 == 235)
		return;

	for (i = 0; i < Global_113648.f_20412.f_145; i = i + 1)
	{
		if (unk_0x4310A0DB886F9FED(&Global_113648.f_20412[i /*16*/], sParam0))
			return;
	}

	if (Global_113648.f_20412.f_145 < 9)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/], sParam0, 16);
		TEXT_LABEL_ASSIGN_STRING(&(Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_4), sParam1, 16);
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_8 = unk_0xA5E11AF0A2B928C1() + iParam3;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_9 = iParam5;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_11 = iParam6;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_12 = iParam2;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_13 = iParam7;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_14 = iParam8;
		Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_15 = iParam9;
	
		if (iParam4 != -1)
			Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_10 = unk_0xA5E11AF0A2B928C1() + iParam3 + iParam4;
		else
			Global_113648.f_20412[Global_113648.f_20412.f_145 /*16*/].f_10 = -1;
	
		Global_113648.f_20412.f_145 = Global_113648.f_20412.f_145 + 1;
		func_19();
	}

	return;
}

void func_19() // Position - 0xF89
{
	int i;

	for (i = 0; i < 3; i = i + 1)
	{
		Global_113648.f_20412.f_146[i] = 0;
	}

	for (i = 0; i < Global_113648.f_20412.f_145; i = i + 1)
	{
		if (IS_BIT_SET(Global_113648.f_20412[i /*16*/].f_11, 0))
			if (Global_113648.f_20412[i /*16*/].f_12 > Global_113648.f_20412.f_146[0])
				Global_113648.f_20412.f_146[0] = Global_113648.f_20412[i /*16*/].f_12;
	
		if (IS_BIT_SET(Global_113648.f_20412[i /*16*/].f_11, 1))
			if (Global_113648.f_20412[i /*16*/].f_12 > Global_113648.f_20412.f_146[1])
				Global_113648.f_20412.f_146[1] = Global_113648.f_20412[i /*16*/].f_12;
	
		if (IS_BIT_SET(Global_113648.f_20412[i /*16*/].f_11, 2))
			if (Global_113648.f_20412[i /*16*/].f_12 > Global_113648.f_20412.f_146[2])
				Global_113648.f_20412.f_146[2] = Global_113648.f_20412[i /*16*/].f_12;
	}

	return;
}

int func_20() // Position - 0x10A0
{
	func_21();

	switch (Global_113648.f_2365.f_539.f_4321)
	{
		case CHAR_MICHAEL:
			return 1;
	
		case CHAR_FRANKLIN:
			return 2;
	
		case CHAR_TREVOR:
			return 4;
	}

	return 0;
}

void func_21() // Position - 0x10E6
{
	eCharacter character;

	if (unk_0x7DE17ACDD8BA2642(unk_0xC1A5EC5C986B98AD()))
	{
		if (_GET_CHARACTER_MODEL(Global_113648.f_2365.f_539.f_4321) != unk_0x504B9BB48D41C264(unk_0xC1A5EC5C986B98AD()))
		{
			character = _GET_PLAYER_CHARACTER_FROM_PED(unk_0xC1A5EC5C986B98AD());
		
			if (func_23(character) && !func_22(14) || Global_112599)
			{
				if (Global_113648.f_2365.f_539.f_4321 != character && func_23(Global_113648.f_2365.f_539.f_4321))
					Global_113648.f_2365.f_539.f_4322 = Global_113648.f_2365.f_539.f_4321;
			
				Global_113648.f_2365.f_539.f_4323 = character;
				Global_113648.f_2365.f_539.f_4321 = character;
				return;
			}
		}
		else
		{
			if (Global_113648.f_2365.f_539.f_4321 != _CHAR_NULL)
				Global_113648.f_2365.f_539.f_4323 = Global_113648.f_2365.f_539.f_4321;
		
			return;
		}
	}

	Global_113648.f_2365.f_539.f_4321 = 145;
	return;
}

BOOL func_22(int iParam0) // Position - 0x11E3
{
	return Global_43257 == iParam0;
}

BOOL func_23(eCharacter echParam0) // Position - 0x11F1
{
	return echParam0 < CHAR_MULTIPLAYER;
}

eCharacter _GET_PLAYER_CHARACTER_FROM_PED(var uParam0) // Position - 0x11FD
{
	eCharacter i;
	int num;

	if (unk_0x7DE17ACDD8BA2642(uParam0))
	{
		num = unk_0x504B9BB48D41C264(uParam0);
	
		for (i = CHAR_MICHAEL; i <= CHAR_TREVOR; i = i + 1)
		{
			if (_GET_CHARACTER_MODEL(i) == num)
				return i;
		}
	}

	return _CHAR_NULL;
}

int _GET_CHARACTER_MODEL(eCharacter character) // Position - 0x123A
{
	if (func_23(character))
		return func_26(character);
	else
		character != _CHAR_NULL;

	return 0;
}

int func_26(eCharacter echParam0) // Position - 0x125F
{
	return Global_2028[echParam0 /*29*/];
}

BOOL func_27(int iParam0) // Position - 0x126E
{
	int num;
	int i;

	num = iParam0;

	for (i = 0; num > 31; i = i + 1)
	{
		num = num - 32;
	}

	if (i < 3)
		return IS_BIT_SET(Global_113648.f_20412.f_150[i], num);

	return false;
}

void func_28(int iParam0, int iParam1) // Position - 0x12AE
{
	unk_0xECDAB41968FF21A8(&Global_113648.f_24997.f_8[iParam0], iParam1);
	return;
}

int func_29(int iParam0) // Position - 0x12C9
{
	int num;

	num = 1;

	switch (iParam0)
	{
		case 1:
			num = 5;
			break;
	
		case 5:
			num = 2;
			break;
	
		case 9:
			num = 10;
			break;
	
		case 11:
			num = 4;
			break;
	
		case 13:
			num = 4;
			break;
	
		case 15:
			num = 2;
			break;
	
		case 17:
			num = 2;
			break;
	
		case 22:
			num = 2;
			break;
	
		case 23:
			num = 2;
			break;
	
		case 25:
			num = 3;
			break;
	
		case 26:
			num = 2;
			break;
	
		case 27:
			num = 2;
			break;
	
		case 29:
			num = 3;
			break;
	}

	return num;
}

int func_30() // Position - 0x137A
{
	var unk;
	int num;

	TEXT_LABEL_ASSIGN_STRING(&unk, unk_0x688846D56810779A(), 64);
	num = func_31(unk);
	return num;
}

int func_31(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12, var uParam13, var uParam14, var uParam15) // Position - 0x1397
{
	switch (unk_0x14580F20CBCE4FA9(&uParam0))
	{
		case joaat("re_abandonedcar"):
			return 23;
	
		case joaat("re_accident"):
			return 0;
	
		case joaat("re_arrests"):
			return 15;
	
		case joaat("re_atmrobbery"):
			return 1;
	
		case joaat("re_bikethief"):
			return 26;
	
		case joaat("re_border"):
			return 29;
	
		case joaat("re_burials"):
			return 24;
	
		case joaat("re_bus_tours"):
			return 2;
	
		case joaat("re_cartheft"):
			return 17;
	
		case joaat("re_chasethieves"):
			return 11;
	
		case joaat("re_crashrescue"):
			return 16;
	
		case joaat("re_cultshootout"):
			return 18;
	
		case joaat("re_dealgonewrong"):
			return 12;
	
		case joaat("re_domestic"):
			return 3;
	
		case joaat("re_drunkdriver"):
			return 27;
	
		case joaat("re_gang_intimidation"):
			return 20;
	
		case joaat("re_gangfight"):
			return 19;
	
		case joaat("re_getaway_driver"):
			return 4;
	
		case joaat("re_hitch_lift"):
			return 13;
	
		case joaat("re_homeland_security"):
			return 28;
	
		case joaat("re_lured"):
			return 7;
	
		case joaat("re_muggings"):
			return 25;
	
		case joaat("re_paparazzi"):
			return 10;
	
		case joaat("re_prisonerlift"):
			return 22;
	
		case joaat("re_prisonvanbreak"):
			return 21;
	
		case joaat("re_securityvan"):
			return 9;
	
		case joaat("re_shoprobbery"):
			return 5;
	
		case joaat("re_snatched"):
			return 6;
	
		case joaat("re_stag_do"):
			return 14;
	
		case joaat("re_yetarian"):
			return 30;
	
		case joaat("re_duel"):
			return 31;
	
		case joaat("re_seaplane"):
			return 32;
	
		case joaat("re_monkey"):
			return 33;
	}

	return -1;
}

void func_32(int iParam0, BOOL bParam1) // Position - 0x1571
{
	if (iParam0 == -1)
		return;

	if (bParam1)
	{
		if (!func_34(iParam0, 0))
		{
			func_33(iParam0, 1, false);
			func_33(iParam0, 2, false);
			func_33(iParam0, 3, false);
			func_33(iParam0, 4, false);
			func_33(iParam0, 0, true);
			Global_77348[iParam0] = 1;
		}
	}
	else
	{
		func_33(iParam0, 0, false);
	}

	return;
}

void func_33(int iParam0, int iParam1, BOOL bParam2) // Position - 0x15CE
{
	if (iParam0 == -1)
		return;

	if (bParam2)
		unk_0xECDAB41968FF21A8(&Global_113648.f_32751[iParam0], iParam1);
	else
		unk_0x061B1200C95204CB(&Global_113648.f_32751[iParam0], iParam1);

	return;
}

BOOL func_34(int iParam0, int iParam1) // Position - 0x1609
{
	if (iParam0 == -1)
		return false;

	return IS_BIT_SET(Global_113648.f_32751[iParam0], iParam1);
}

void func_35() // Position - 0x1629
{
	int i;
	BOOL flag;
	int j;

	switch (iLocal_48)
	{
		case 0:
			func_69();
			func_68(7);
			unk_0x852EC2A7DE66202D(iLocal_154);
			iLocal_48 = 1;
			break;
	
		case 1:
			switch (iLocal_148)
			{
				case 0:
					if (unk_0x0CBB7C273DED26E7(iLocal_154))
					{
						unk_0x852EC2A7DE66202D(iLocal_153);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 1:
					if (unk_0x0CBB7C273DED26E7(iLocal_153))
					{
						unk_0x852EC2A7DE66202D(iLocal_155);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 2:
					if (unk_0x0CBB7C273DED26E7(iLocal_155))
					{
						unk_0x852EC2A7DE66202D(iLocal_156);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 3:
					if (unk_0x0CBB7C273DED26E7(iLocal_156))
					{
						unk_0x852EC2A7DE66202D(iLocal_157);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 4:
					if (unk_0x0CBB7C273DED26E7(iLocal_157))
					{
						unk_0x7F2BD159A7FF94C0(sLocal_137);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 5:
					if (unk_0xC546C50EE3EA181E(sLocal_137))
					{
						unk_0x7F2BD159A7FF94C0(sLocal_138);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 6:
					if (unk_0x0CBB7C273DED26E7(iLocal_154) && unk_0x0CBB7C273DED26E7(iLocal_153) && unk_0x0CBB7C273DED26E7(iLocal_155) && unk_0x0CBB7C273DED26E7(iLocal_157) && unk_0x0CBB7C273DED26E7(iLocal_156) && unk_0xC546C50EE3EA181E(sLocal_137) && unk_0xC546C50EE3EA181E(sLocal_138))
					{
						func_64(0, uLocal_139, -1f, true);
						unk_0xE0A291F406691F03(iLocal_153);
						func_64(1, uLocal_139, -1082130432, false);
						func_64(2, uLocal_139, -1082130432, false);
					
						for (i = 0; i <= 4; i = i + 1)
						{
							func_55(i, false, uLocal_139, -1082130432);
						}
					
						func_45(true);
						iLocal_148 = iLocal_148 + 1;
					}
					break;
			
				case 7:
					if (func_44())
					{
						func_43(&(uLocal_116[0 /*4*/].f_1));
						func_68(1);
						unk_0x0133FF6E23A1DCA4(5, joaat("PLAYER"), uLocal_171);
						unk_0x0133FF6E23A1DCA4(5, uLocal_171, joaat("PLAYER"));
						func_64(3, uLocal_139, -1082130432, false);
						func_64(4, uLocal_139, -1082130432, false);
						unk_0xE0A291F406691F03(iLocal_154);
						unk_0xE0A291F406691F03(iLocal_156);
						func_55(7, false, uLocal_139, -1082130432);
						func_55(8, false, uLocal_139, -1082130432);
						func_55(9, false, uLocal_139, -1082130432);
						func_55(10, false, uLocal_139, -1082130432);
						unk_0xE0A291F406691F03(iLocal_155);
						unk_0xE0A291F406691F03(iLocal_157);
						iLocal_148 = iLocal_148 + 1;
					}
				
					if (func_42())
					{
						if (!unk_0x1774A64AB2C276EF(unk_0xC1A5EC5C986B98AD()))
						{
							unk_0x0133FF6E23A1DCA4(5, joaat("PLAYER"), uLocal_171);
							unk_0x0133FF6E23A1DCA4(5, uLocal_171, joaat("PLAYER"));
						}
						else if (!func_121(2))
						{
							unk_0x0133FF6E23A1DCA4(3, joaat("PLAYER"), uLocal_171);
							unk_0x0133FF6E23A1DCA4(3, uLocal_171, joaat("PLAYER"));
						}
					}
					break;
			
				case 8:
					flag = true;
				
					for (j = 0; j <= 10; j = j + 1)
					{
						if (!func_39(j, false))
							flag = false;
					}
				
					if (flag)
						iLocal_148 = iLocal_148 + 1;
					break;
			
				case 9:
					if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]))
						if (func_38(unk_0xC1A5EC5C986B98AD(), uLocal_145, 1) >= 200f && func_38(uLocal_116[0 /*4*/], uLocal_145, 1) >= 200f)
							iLocal_48 = 2;
					else
						func_37();
					break;
			}
			break;
	
		case 2:
			func_36(1);
			break;
	}

	return;
}

void func_36(int iParam0) // Position - 0x1968
{
	iLocal_148 = 0;
	iLocal_48 = 0;
	iLocal_47 = iParam0;
	return;
}

void func_37() // Position - 0x197A
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]))
		unk_0x439314AD4FC79186(uLocal_116[0 /*4*/], 2);

	func_169(true, false);
	return;
}

float func_38(var uParam0, var uParam1, var uParam2, var uParam3, int iParam4) // Position - 0x199E
{
	var unk;

	if (!unk_0x055111B11E6624FD(uParam0, 0))
		unk = { unk_0x30BE8A934C020461(uParam0, 1) };
	else
		unk = { unk_0x30BE8A934C020461(uParam0, 0) };

	return unk_0x2E496FE654DA4166(unk, uParam1, iParam4);
}

BOOL func_39(int iParam0, BOOL bParam1) // Position - 0x19D8
{
	int num;

	if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[iParam0 /*6*/]))
	{
		return true;
	}
	else
	{
		if (func_41(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 1) >= 400f)
		{
			func_40(iParam0, false);
			return true;
		}
	
		if (bParam1)
		{
			num = iParam0;
		
			if (num >= 7 && !unk_0xCE4AAA035282336C(uLocal_49[iParam0 /*6*/].f_1))
				return true;
		}
	}

	return false;
}

void func_40(int iParam0, BOOL bParam1) // Position - 0x1A3F
{
	if (unk_0x7DE17ACDD8BA2642(uLocal_49[iParam0 /*6*/]))
	{
		func_43(&(uLocal_49[iParam0 /*6*/].f_1));
	
		if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[iParam0 /*6*/]) && unk_0xDC116D955A63BDBE(uLocal_49[iParam0 /*6*/]) && !unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 1) && !unk_0xACD39355028D39EF(uLocal_49[iParam0 /*6*/]) && !unk_0x5CD470B5BDDAC029(uLocal_49[iParam0 /*6*/]))
			unk_0xA81AA70A4D25E140(uLocal_49[iParam0 /*6*/], 1, 1);
	
		if (bParam1)
			unk_0xE9C250FA35A936C8(&uLocal_49[iParam0 /*6*/]);
		else
			unk_0x1EEF71E3CDD868D3(&uLocal_49[iParam0 /*6*/]);
	
		uLocal_49[iParam0 /*6*/].f_3 = 0;
		uLocal_49[iParam0 /*6*/].f_2 = 0;
	}

	return;
}

float func_41(var uParam0, var uParam1, int iParam2) // Position - 0x1AE7
{
	var unk;
	var unk4;

	if (!unk_0x055111B11E6624FD(uParam0, 0))
		unk = { unk_0x30BE8A934C020461(uParam0, 1) };
	else
		unk = { unk_0x30BE8A934C020461(uParam0, 0) };

	if (!unk_0x055111B11E6624FD(uParam1, 0))
		unk4 = { unk_0x30BE8A934C020461(uParam1, 1) };
	else
		unk4 = { unk_0x30BE8A934C020461(uParam1, 0) };

	return unk_0x2E496FE654DA4166(unk, unk4, iParam2);
}

BOOL func_42() // Position - 0x1B45
{
	return unk_0x9DC9E896F189AAA5(unk_0xC1A5EC5C986B98AD(), 3150.5076f, 2179.3433f, -3.388727f, 3064.6077f, 2223.616f, 12.381769f, 103f, 0, 1, 0);
}

void func_43(var uParam0) // Position - 0x1B7B
{
	if (unk_0xCE4AAA035282336C(*uParam0))
	{
		unk_0xBD91E7D4B770F97E(*uParam0, 0);
		unk_0x45533C09A6C9B409(uParam0);
	}

	return;
}

BOOL func_44() // Position - 0x1B9B
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]) && unk_0xE0D346789C5160EB(unk_0xC1A5EC5C986B98AD(), uLocal_116[0 /*4*/], 0))
		return true;

	return false;
}

int func_45(BOOL bParam0) // Position - 0x1BC7
{
	if (func_49())
	{
		Global_113638 = true;
		Global_113635 = unk_0xA5E11AF0A2B928C1();
	
		if (func_15(Global_113637))
			func_46(0);
	
		unk_0x97DBA2AA6C70AFC7(1, "RE_TITLE" /*Random Event*/);
	
		if (bParam0 && func_15(Global_113637))
			unk_0x01680CFD15C1C85D();
	
		return 1;
	}

	return 0;
}

void func_46(int iParam0) // Position - 0x1C19
{
	switch (iParam0)
	{
		case 0:
			if (Global_113648.f_24997.f_2 < 3)
			{
				if (!unk_0x5835DDABA96BB199())
				{
					_DISPLAY_HELP_TEXT(func_48(iParam0), -1);
					Global_113648.f_24997.f_2 = Global_113648.f_24997.f_2 + 1;
					unk_0xECDAB41968FF21A8(&Global_113644, 0);
				}
			}
			break;
	
		case 1:
			if (!IS_BIT_SET(Global_113644, 1))
			{
				if (!unk_0x5835DDABA96BB199())
				{
					_DISPLAY_HELP_TEXT(func_48(iParam0), -1);
					Global_113648.f_24997.f_3 = Global_113648.f_24997.f_3 + 1;
					unk_0xECDAB41968FF21A8(&Global_113644, 1);
				}
			}
			break;
	
		case 2:
			if (!IS_BIT_SET(Global_113644, 2))
			{
				if (!unk_0x5835DDABA96BB199())
				{
					_DISPLAY_HELP_TEXT(func_48(iParam0), -1);
					Global_113648.f_24997.f_4 = Global_113648.f_24997.f_4 + 1;
					unk_0xECDAB41968FF21A8(&Global_113644, 2);
				}
			}
			break;
	}

	return;
}

void _DISPLAY_HELP_TEXT(char* text, int iParam1) // Position - 0x1CF4
{
	unk_0x476CC3711883B966(text);
	unk_0xBC8969B539DCA239(0, 0, 1, iParam1);
	return;
}

char* func_48(int iParam0) // Position - 0x1D0B
{
	char* str;

	str = "";

	switch (iParam0)
	{
		case 0:
			str = "AM_H_REFS" /*Various events unfold across San Andreas daily. These events will become blipped on the Radar when nearby.*/;
			break;
	
		case 1:
			str = "RE_FLASHBLIP" /*Flashing blue and red blips indicate situations around San Andreas that you can choose to help with.*/;
			break;
	
		case 2:
			str = "RE_HANDOVER" /*If you retrieve a stolen item, you can choose to keep it or return it for a reward.*/;
			break;
	}

	return str;
}

BOOL func_49() // Position - 0x1D4B
{
	switch (func_50(&Global_32223, 0, 5, false, unk_0xAF908D5E2416DA93()))
	{
		case 1:
			return true;
	
		case 0:
			return true;
	}

	return false;
}

int func_50(var uParam0, int iParam1, int iParam2, BOOL bParam3, int iParam4) // Position - 0x1D81
{
	int i;

	if (iParam1 == 7)
		return 0;

	if (!bParam3)
		if (Global_98159.f_44 == 1)
			return 2;

	if (iParam1 == 0)
	{
		if (func_54(0))
			return 0;
	
		Global_43221 = Global_43221 + 1;
		*uParam0 = Global_43221;
		unk_0xA7225B88CE344621(unk_0x5DC3DCA82C806319(), 0);
		Global_23131.f_5 = 0;
	
		if (iParam2 != 5)
			unk_0xDAD6345C5D317E79(8);
	
		Global_43257 = iParam2;
		Global_43219 = *uParam0;
		Global_43220 = iParam4;
		Global_43218 = 0;
		return 1;
	}

	if (*uParam0 != -1)
	{
		if (Global_43218 > 0)
		{
			i = 0;
		
			for (i = 0; i < Global_43218; i = i + 1)
			{
				if (Global_43224[i /*4*/] == *uParam0)
					return 2;
			}
		}
		else if (Global_43219 == *uParam0)
		{
			return 1;
		}
	
		*uParam0 = -1;
	}

	if (*uParam0 == -1)
	{
		if (!_CAN_ENTER_FREEROAM_STATE(iParam2))
			return 0;
	
		if (Global_43218 == 8)
			return 0;
	
		Global_43221 = Global_43221 + 1;
		*uParam0 = Global_43221;
		Global_43224[Global_43218 /*4*/] = Global_43221;
		Global_43224[Global_43218 /*4*/].f_1 = iParam1;
		Global_43224[Global_43218 /*4*/].f_2 = iParam2;
		Global_43224[Global_43218 /*4*/].f_3 = 0;
		Global_43218 = Global_43218 + 1;
	
		if (iParam4 != 0)
			func_51(uParam0, iParam4);
	}

	return 2;
}

void func_51(var uParam0, int iParam1) // Position - 0x1EB8
{
	int i;

	if (Global_43218 == 0)
		return;

	if (*uParam0 == -1)
		return;

	i = 0;

	for (i = 0; i < Global_43218; i = i + 1)
	{
		if (Global_43224[i /*4*/] == *uParam0)
			Global_43224[i /*4*/].f_3 = iParam1;
	}

	*uParam0 = -1;
	return;
}

BOOL _CAN_ENTER_FREEROAM_STATE(int iParam0) // Position - 0x1F07
{
	return func_53(iParam0, Global_43257);
}

BOOL func_53(int iParam0, int iParam1) // Position - 0x1F18
{
	if (iParam1 == 15)
		return 1;

	if (iParam0 == 15)
		return 0;

	switch (iParam0)
	{
		case 16:
			switch (iParam1)
			{
				case 9:
				case 10:
				case 7:
				case 13:
				case 14:
					return 0;
			}
		
			return 1;
	
		case 0:
			switch (iParam1)
			{
				case 5:
				case 17:
					return 1;
			}
			break;
	
		case 2:
		case 3:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
				case 17:
					return 1;
			}
			break;
	
		case 4:
			if (iParam1 == 17)
				return 1;
			break;
	
		case 5:
			break;
	
		case 6:
		case 8:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 7:
			if (iParam1 == 6)
				return 1;
			break;
	
		case 9:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 10:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 17:
					return 1;
			}
			break;
	
		case 11:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 17:
			switch (iParam1)
			{
				case 17:
				case 12:
				case 5:
					return 1;
			}
			break;
	
		case 18:
		case 12:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
					return 1;
			}
			break;
	
		case 13:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	
		case 14:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	}

	return 0;
}

BOOL func_54(int iParam0) // Position - 0x20F9
{
	if (Global_43257 == 15)
		return false;

	if (_CAN_ENTER_FREEROAM_STATE(iParam0))
		return false;

	return true;
}

int func_55(int iParam0, BOOL bParam1, var uParam2, var uParam3, var uParam4, int iParam5) // Position - 0x211B
{
	var unk;
	float num;
	int num2;
	int num3;
	int num4;
	int num5;
	var unk4;
	var unk5;
	var unk8;
	int num6;
	int num7;
	int num8;
	int num9;
	int num10;
	float num11;
	float num12;
	float num13;
	float num14;
	float num15;
	float num16;
	float num17;
	float num18;
	var unk11;
	int num19;
	int num20;
	int num21;
	int num22;
	int num23;
	BOOL flag;
	int num24;
	int num25;
	int num26;
	int num27;
	int num28;
	int num29;
	BOOL flag2;
	int num30;
	int num31;
	int num32;
	int num33;
	int num34;
	int num35;
	int num36;
	int num37;
	int num38;
	int num39;
	int num40;
	int num41;
	int num42;
	int num43;
	int num44;
	int num45;
	int num46;
	int num47;
	int num48;
	BOOL flag3;
	BOOL flag4;
	int num49;
	int num50;
	BOOL flag5;
	BOOL flag6;
	int num51;
	BOOL flag7;
	int num52;
	BOOL flag8;
	BOOL flag9;
	int num53;
	int num54;
	char* str;

	if (!unk_0x7DE17ACDD8BA2642(uLocal_49[iParam0 /*6*/]))
	{
		num3 = joaat("WEAPON_UNARMED");
		num4 = 0;
		num5 = -1;
		unk5 = { uLocal_139 };
		unk8 = { uLocal_139 };
		num6 = 0;
		num7 = 1;
		num8 = 0;
		num9 = 0;
		num10 = 1;
		num11 = 0.75f;
		num12 = 0.95f;
		num13 = 60f;
		num14 = 5f;
		num15 = 120f;
		num16 = -90f;
		num17 = 90f;
		num18 = 125f;
		unk11 = uLocal_171;
		num19 = 1;
		num20 = 1;
		num21 = 0;
		num22 = 1;
		num23 = 1;
		flag = false;
		num24 = 0;
		num25 = 0;
		num26 = 0;
		num27 = 1;
		num28 = 1;
		num29 = 1;
		flag2 = false;
		num30 = 1;
		num31 = 0;
		num32 = 1;
		num33 = 1;
		num34 = 1;
		num35 = 1;
		num36 = 1;
		num37 = 1;
		num38 = 0;
		num39 = 0;
		num40 = 0;
		num41 = 0;
		num42 = 0;
		num43 = 0;
		num44 = 1;
		num45 = 0;
		num46 = 0;
		num47 = 0;
		num48 = 1;
		flag3 = true;
		flag4 = false;
		num49 = 0;
		num50 = 0;
		flag5 = false;
		flag6 = false;
		num51 = 0;
		flag7 = false;
		num52 = 0;
		flag8 = false;
		flag9 = false;
		num53 = 0;
		num54 = 0;
	
		switch (iParam0)
		{
			case 0:
				num2 = iLocal_157;
				num4 = uLocal_116[1 /*4*/];
				num3 = joaat("WEAPON_PISTOL");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 1:
				num2 = iLocal_157;
				unk = { unk_0x7394CB479CCA24AF(uLocal_116[1 /*4*/], 0f, -1.7f, 0.9f) };
				num = unk_0xD850DD08D5434072(uLocal_116[1 /*4*/]) + 90f;
				str = "WORLD_HUMAN_HANG_OUT_STREET";
				num3 = joaat("WEAPON_MICROSMG");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 2:
				num2 = iLocal_155;
				num4 = uLocal_116[2 /*4*/];
				num3 = joaat("WEAPON_PISTOL");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 3:
				num2 = iLocal_155;
				unk = { unk_0x7394CB479CCA24AF(uLocal_116[2 /*4*/], 0f, -1.5f, 0.9f) };
				num = unk_0xD850DD08D5434072(uLocal_116[2 /*4*/]) + 90f;
				str = "WORLD_HUMAN_HANG_OUT_STREET";
				num3 = joaat("WEAPON_MICROSMG");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 4:
				num2 = iLocal_155;
				unk = { unk_0x7394CB479CCA24AF(uLocal_116[2 /*4*/], 0f, -2.3f, 0.9f) };
				num = unk_0xD850DD08D5434072(uLocal_116[2 /*4*/]) + 90f;
				str = "WORLD_HUMAN_HANG_OUT_STREET";
				num3 = joaat("WEAPON_PISTOL");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 7:
				num2 = iLocal_155;
				num4 = uLocal_116[3 /*4*/];
				num3 = joaat("WEAPON_PISTOL");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 8:
				num2 = iLocal_155;
				num4 = uLocal_116[3 /*4*/];
				num5 = 0;
				num3 = joaat("WEAPON_MICROSMG");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 9:
				num2 = iLocal_157;
				num4 = uLocal_116[4 /*4*/];
				num3 = joaat("WEAPON_PISTOL");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			case 10:
				num2 = iLocal_157;
				num4 = uLocal_116[4 /*4*/];
				num5 = 0;
				num3 = joaat("WEAPON_MICROSMG");
				num26 = 1;
				num27 = 0;
				num37 = 0;
				num53 = 1;
				break;
		
			default:
				break;
		}
	
		if (!unk_0x0CBB7C273DED26E7(num2))
			return 0;
	
		if (!func_63(uParam2, uLocal_139, false))
			if (!unk_0x7DE17ACDD8BA2642(unk4))
				unk = { uParam2 };
			else
				unk5 = { uParam2 };
	
		if (iParam5 != -1f)
			num = iParam5;
	
		if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(num4))
		{
			uLocal_49[iParam0 /*6*/] = unk_0x4FA078B1B32445FD(num4, 26, num2, num5, 1, 1);
		}
		else
		{
			uLocal_49[iParam0 /*6*/] = unk_0x69FDD9508259E5B5(26, num2, unk, num, 1, 1);
			unk_0xDDE449983CE7572E(uLocal_49[iParam0 /*6*/], num30, 0);
			unk_0x362A7D3F1F776CB5(uLocal_49[iParam0 /*6*/], !flag6, 0);
		}
	
		if (flag2)
			unk_0x2EDDA32F6EF3C8B7(uLocal_49[iParam0 /*6*/], unk, 0, 0, 1);
	
		if (bParam1)
			if (unk11 == uLocal_171)
				uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], true, 145);
			else
				uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], false, 145);
	
		unk_0x6925F2023EB53DDE(uLocal_49[iParam0 /*6*/], num48);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 213, num19);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 212, num20);
		unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], num21);
		unk_0x71A535529C1CA5DF(uLocal_49[iParam0 /*6*/], num22);
		unk_0xB7561E3CA1BE2262(uLocal_49[iParam0 /*6*/], num23);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 188, num44);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 42, !flag);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 9, num25);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 3, num27);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 342, num46);
		unk_0x351E6B9FD37CFF33(uLocal_49[iParam0 /*6*/], num26, 1);
		unk_0x92E052CD54EA74BF(uLocal_49[iParam0 /*6*/], num9);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 8, num29);
		unk_0x724FFAED1C56CE2B(uLocal_49[iParam0 /*6*/], num28, 0);
		unk_0x226C3A30511D5DC4(uLocal_49[iParam0 /*6*/], num31);
		unk_0xCBDC2B59922F92C3(uLocal_49[iParam0 /*6*/], num3, -1, num24, num24);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 2, num32);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, num33);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 52, num34);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 13, num52);
		unk_0x7D519820039E4BE9(uLocal_49[iParam0 /*6*/], num35);
		unk_0xD14067404D35AAE7(uLocal_49[iParam0 /*6*/], num36);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 118, num37);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 115, num38);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 12, num39);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 36, num40);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 35, num41);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 29, num42);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 185, num43);
		unk_0xBE9E8C86A1CE952F(uLocal_49[iParam0 /*6*/], num45);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 17, num49);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 5, num53);
		unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 46, num54);
		unk_0xF7A339DDAF217914(uLocal_49[iParam0 /*6*/], num50);
		unk_0x57B1A0850EB7F1F4(uLocal_49[iParam0 /*6*/], num47);
		func_59(uLocal_49[iParam0 /*6*/], num13, num14, num15, num16, num17);
		unk_0x2718E9CC165A99F6(uLocal_49[iParam0 /*6*/], num51);
		unk_0x598D91BBD045C1F3(uLocal_49[iParam0 /*6*/], 203, flag7);
		unk_0x627B62B04769B4B4(uLocal_49[iParam0 /*6*/], !flag7);
	
		if (!flag3)
			func_58(iParam0, 2);
	
		if (!unk_0x2AC37494BBF1DB16(str))
			unk_0x2B8574FAEA21141B(uLocal_49[iParam0 /*6*/], str, -1, 0);
	
		if (num8 > 0)
		{
			if (num8 > unk_0x15848AD475FC952B(uLocal_49[iParam0 /*6*/]) || flag4)
				unk_0x84D39C7915D8BEE0(uLocal_49[iParam0 /*6*/], num8);
		
			unk_0xED2BD879E739E86B(uLocal_49[iParam0 /*6*/], num8, 0);
		}
	
		if (num10 > 0)
			unk_0x55B25C3B4CEEEF4B(uLocal_49[iParam0 /*6*/], num10);
	
		if (num18 >= 0f)
			unk_0xF70E5B487D125DA0(uLocal_49[iParam0 /*6*/], num18);
	
		if (num11 > 0f || num12 > 0f)
			unk_0xBE7D19843CA56CAA(uLocal_49[iParam0 /*6*/], num11, num12);
	
		if (flag8)
			unk_0x002C268EFF2A6ADE(uLocal_49[iParam0 /*6*/], unk, 5f, 0, 0);
	
		if (flag9)
			unk_0x65B2119721152DC2(uLocal_49[iParam0 /*6*/]);
	
		unk_0x8386356641D0DED1(uLocal_49[iParam0 /*6*/], unk11);
	
		if (unk_0x7DE17ACDD8BA2642(unk4))
		{
			unk_0xBD02C9D4D7076ECC(uLocal_49[iParam0 /*6*/], unk4, -1, unk5, unk8, 1, 1, 1, 0, 2, 1, 0);
			unk_0xAC2C285C82A9244C(uLocal_49[iParam0 /*6*/], 0);
			func_58(iParam0, 1);
		}
	
		func_57(iParam0);
		func_56(iParam0);
	
		if (num6 != 0)
		{
			unk_0x25653C7E6836F120(uLocal_49[iParam0 /*6*/], num3, num6);
			unk_0x8CC688927B10006C(uLocal_49[iParam0 /*6*/], num3, 1);
		}
	
		unk_0xE5069E489A2DE9E5(uLocal_49[iParam0 /*6*/], num7);
	
		if (flag5)
			unk_0x0002182D625AEFEA(uLocal_49[iParam0 /*6*/], 1, "DEFAULT_ACTION");
	
		return 1;
	}

	return 0;
}

void func_56(int iParam0) // Position - 0x285C
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[iParam0 /*6*/]))
	{
		switch (iParam0)
		{
			case 0:
				break;
		}
	}

	return;
}

void func_57(int iParam0) // Position - 0x2881
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[iParam0 /*6*/]))
	{
		switch (iParam0)
		{
			case 0:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 0, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 0, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 0, 0);
				break;
		
			case 1:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 1, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 1, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 1, 0);
				break;
		
			case 2:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 0, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 0, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 0, 0);
				break;
		
			case 3:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 1, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 1, 0, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 1, 0);
				break;
		
			case 4:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 1, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 0, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 1, 0, 0);
				break;
		
			case 7:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 0, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 1, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 1, 2, 0);
				break;
		
			case 8:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 1, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 0, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 2, 0);
				break;
		
			case 9:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 1, 2, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 0, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 0, 2, 0);
				break;
		
			case 10:
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 0, 0, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 3, 1, 1, 0);
				unk_0xFECAE061D8C80757(uLocal_49[iParam0 /*6*/], 4, 1, 0, 0);
				break;
		}
	}

	return;
}

void func_58(int iParam0, int iParam1) // Position - 0x2A68
{
	unk_0xECDAB41968FF21A8(&(uLocal_49[iParam0 /*6*/].f_2), iParam1);
	return;
}

void func_59(var uParam0, float fParam1, float fParam2, float fParam3, float fParam4, float fParam5) // Position - 0x2A7E
{
	if (!unk_0x66599E73DBA5A850(uParam0))
	{
		unk_0xEA1F0CBFFACFA528(uParam0, fParam1);
		unk_0x6B7D33F46CAEAA6E(uParam0, fParam2);
		unk_0xBA06A80FD83D2B7D(uParam0, fParam3 / 2f);
		unk_0x5A2D707ED27D8B66(uParam0, fParam4);
		unk_0x04795F1D2B04EDC7(uParam0, fParam5);
	}

	return;
}

int func_60(var uParam0, BOOL bParam1, int iParam2) // Position - 0x2ABA
{
	int num;

	num = func_61(uParam0, !bParam1, false);

	if (iParam2 != 145 && unk_0xCE4AAA035282336C(num) && unk_0x42FA33BDEDF21186(&(Global_2028[iParam2 /*29*/].f_3)))
		unk_0x4049FDC177C92D4B(num, &(Global_2028[iParam2 /*29*/].f_3));

	return num;
}

int func_61(var uParam0, BOOL bParam1, BOOL bParam2) // Position - 0x2B04
{
	int num;

	if (!unk_0x7DE17ACDD8BA2642(uParam0))
		return 0;

	num = unk_0x53B496178AE44636(uParam0);

	if (unk_0xA6B591D40DE982B5(uParam0))
	{
		unk_0x225EB85DBC38E707(num, unk_0xA26A9A07F761D8F8() ? 1f : 1f);
	
		if (!bParam2)
			unk_0x3BCF1F6A3573A1DF(num, bParam1);
		else
			unk_0x1886753DA39F38F8(num, 2);
	}
	else if (unk_0x71904BD37B848CAF(uParam0))
	{
		unk_0x225EB85DBC38E707(num, unk_0xA26A9A07F761D8F8() ? 0.7f : 0.7f);
		unk_0x3BCF1F6A3573A1DF(num, bParam1);
	}
	else if (unk_0x4E7A9806173C3AD1(uParam0))
	{
		unk_0x225EB85DBC38E707(num, unk_0xA26A9A07F761D8F8() ? 0.7f : 0.7f);
	}

	return num;
}

var func_62(BOOL bParam0, var uParam1, var uParam2) // Position - 0x2BA8
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

BOOL func_63(float fParam0, var uParam1, var uParam2, float fParam3, float fParam4, float fParam5, BOOL bParam6) // Position - 0x2BBF
{
	if (bParam6)
		return fParam0 == fParam3 && fParam0.f_1 == fParam3.f_1;

	return fParam0 == fParam3 && fParam0.f_1 == fParam3.f_1 && fParam0.f_2 == fParam3.f_2;
}

int func_64(int iParam0, var uParam1, var uParam2, var uParam3, int iParam4, BOOL bParam5) // Position - 0x2C06
{
	var unk;
	float num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;
	int num7;
	int num8;
	BOOL flag;
	BOOL flag2;
	BOOL flag3;
	int num9;
	int num10;
	int num11;
	int num12;
	int num13;
	int num14;
	int num15;
	BOOL flag4;
	int num16;
	float num17;
	float num18;
	float num19;
	var unk4;
	var unk5;
	var unk8;
	var unk11;

	if (!unk_0x7DE17ACDD8BA2642(uLocal_116[iParam0 /*4*/]))
	{
		num3 = 0;
		num4 = 1;
		num5 = 1;
		num6 = 0;
		num7 = 0;
		num8 = 1;
		flag = false;
		flag2 = false;
		flag3 = true;
		num9 = 0;
		num10 = 0;
		num11 = 0;
		num12 = 0;
		num13 = 0;
		num14 = 1;
		num15 = 0;
		flag4 = false;
		num16 = 1;
		num18 = -1f;
		num19 = -1f;
	
		switch (iParam0)
		{
			case 0:
				unk = { uLocal_145 };
				num = 163.5528f;
				num2 = iLocal_153;
				flag3 = false;
				num13 = 1;
				num14 = 0;
				flag4 = true;
				num6 = 1;
				break;
		
			case 1:
				unk = { 3087.114f, 2195.558f, 2f };
				num = 157.68f;
				num2 = iLocal_156;
				num15 = 1;
				flag = true;
				flag4 = true;
				num16 = 0;
				num13 = 1;
				num14 = 0;
				break;
		
			case 2:
				unk = { 3092.788f, 2197.243f, 2f };
				num = -22.32f;
				num2 = iLocal_154;
				num15 = 1;
				flag = true;
				flag4 = true;
				num16 = 0;
				num13 = 1;
				num14 = 0;
				break;
		
			case 3:
				unk = { 3170.4011f, 1964.5874f, 0.2889f };
				num = 341.1273f;
				num2 = iLocal_154;
				num15 = 1;
				flag = true;
				flag4 = true;
				num16 = 0;
				num13 = 1;
				num14 = 0;
				break;
		
			case 4:
				unk = { 3227.12f, 2074.91f, 0.75f };
				num = 175.16f;
				num2 = iLocal_156;
				num15 = 1;
				flag = true;
				flag4 = true;
				num16 = 0;
				num13 = 1;
				num14 = 0;
				break;
		
			default:
				break;
		}
	
		if (!unk_0x0CBB7C273DED26E7(num2))
			return 0;
	
		if (!func_63(uParam1, uLocal_139, false))
			unk = { uParam1 };
	
		if (iParam4 != -1f)
			num = iParam4;
	
		uLocal_116[iParam0 /*4*/] = unk_0xABEEDBEF2BBDF5B3(num2, unk, num, 1, 1, 0);
		func_67(iParam0);
		func_66(iParam0);
		unk_0x724FFAED1C56CE2B(uLocal_116[iParam0 /*4*/], num5, 0);
	
		if (unk_0x7DE17ACDD8BA2642(unk4))
			unk_0xBD02C9D4D7076ECC(uLocal_116[iParam0 /*4*/], unk4, -1, unk5, unk8, 0, 0, 0, 0, 2, 1, 0);
		else
			unk_0xDDE449983CE7572E(uLocal_116[iParam0 /*4*/], num4, 0);
	
		unk_0xCCF0ADCE322CD93F(uLocal_116[iParam0 /*4*/], num6);
		unk_0x2718E9CC165A99F6(uLocal_116[iParam0 /*4*/], num7);
		unk_0x10A3F931EEFE03E8(uLocal_116[iParam0 /*4*/], num8);
	
		if (flag)
			unk_0xC8EE2E23B410B65A(uLocal_116[iParam0 /*4*/], flag, 1, 0);
	
		unk_0x226C3A30511D5DC4(uLocal_116[iParam0 /*4*/], num9);
		unk_0x351E6B9FD37CFF33(uLocal_116[iParam0 /*4*/], num10, 1);
		unk_0xC996F5AD4F447BE3(uLocal_116[iParam0 /*4*/], num11);
		unk_0xF5188E9E8E0A9F2A(uLocal_116[iParam0 /*4*/], num12);
		unk_0x495ED6976429C1E8(uLocal_116[iParam0 /*4*/], num14, 0);
		unk_0x9D92700BDDC6B69B(uLocal_116[iParam0 /*4*/], num13);
		unk_0x52B0DD24AA97ABDB(uLocal_116[iParam0 /*4*/], num15);
		unk_0x53A563E258997DA8(uLocal_116[iParam0 /*4*/], num16);
	
		if (!unk_0x2AC37494BBF1DB16(unk11))
			unk_0x33A7FF7A4EECCA59(uLocal_116[iParam0 /*4*/], unk11);
	
		if (num17 > 0f)
			unk_0xC75D5CC27D850469(uLocal_116[iParam0 /*4*/], num17);
	
		if (flag2)
			unk_0xD1AD77A7D621DF91(uLocal_116[iParam0 /*4*/]);
	
		if (num18 >= 0f)
			unk_0x0AD2C204FFF30CE9(uLocal_116[iParam0 /*4*/], num18);
	
		if (num19 >= 0f)
			unk_0x14324B739A22D42A(uLocal_116[iParam0 /*4*/], num19);
	
		if (flag4)
			unk_0xA1FC89AFB27CD4A4(uLocal_116[iParam0 /*4*/], 1);
	
		if (num3 != 0)
			unk_0x439314AD4FC79186(uLocal_116[iParam0 /*4*/], num3);
	
		if (bParam5)
			uLocal_116[iParam0 /*4*/].f_1 = func_65(uLocal_116[iParam0 /*4*/], flag3, false);
	
		if (iParam0 == 0)
			unk_0xD1081E1D2877ED71(uLocal_116[iParam0 /*4*/], 3);
	}

	return 1;
}

int func_65(var uParam0, BOOL bParam1, BOOL bParam2) // Position - 0x2F44
{
	return func_61(uParam0, !bParam1, bParam2);
}

void func_66(int iParam0) // Position - 0x2F57
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[iParam0 /*4*/]))
	{
		switch (iParam0)
		{
			case 2:
			case 3:
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 1, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 2, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 3, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 4, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 5, 0);
				break;
		
			case 1:
			case 4:
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 1, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 2, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 3, 1);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 4, 0);
				unk_0x0D14FEE68F4897C9(uLocal_116[iParam0 /*4*/], 5, 1);
				break;
		}
	}

	return;
}

void func_67(int iParam0) // Position - 0x3009
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[iParam0 /*4*/]))
	{
		switch (iParam0)
		{
			case 1:
				unk_0xDED5B7E9594C5D3B(uLocal_116[iParam0 /*4*/], 111, 28);
				unk_0xDC951F1A74E9384D(uLocal_116[iParam0 /*4*/], 111, 156);
				break;
		
			case 2:
				unk_0xDED5B7E9594C5D3B(uLocal_116[iParam0 /*4*/], 121, 20);
				unk_0xDC951F1A74E9384D(uLocal_116[iParam0 /*4*/], 0, 156);
				break;
		
			case 3:
				unk_0xDED5B7E9594C5D3B(uLocal_116[iParam0 /*4*/], 121, 46);
				unk_0xDC951F1A74E9384D(uLocal_116[iParam0 /*4*/], 0, 156);
				break;
		
			case 4:
				unk_0xDED5B7E9594C5D3B(uLocal_116[iParam0 /*4*/], 111, 54);
				unk_0xDC951F1A74E9384D(uLocal_116[iParam0 /*4*/], 112, 156);
				break;
		}
	}

	return;
}

void func_68(int iParam0) // Position - 0x30B1
{
	unk_0xECDAB41968FF21A8(&iLocal_149, iParam0);
	return;
}

void func_69() // Position - 0x30C1
{
	if (unk_0x48DC6A21D1CAD89C(unk_0x93E99A2DBCBA9CFA()))
		unk_0xE333240A90F2FF3C(unk_0x93E99A2DBCBA9CFA(), 1, 0);

	_DEBUG_PRINT(1);
	unk_0xD3EC2BD7C1DEA1A7(1);
	func_80(0);
	unk_0x8E09E8C52602EBB2(3073.4102f, 2161.6038f, -20f, 3123.8157f, 2216.8997f, 9.0777f, 0, 1);
	unk_0xC6AD633F6B896D88(3073.4102f, 2161.6038f, -20f, 3123.8157f, 2216.8997f, 9.0777f, 0);

	if (!unk_0x67670574D396B9A8(3084.41f, 2080.39f, 0f, 2f))
	{
		unk_0x8E09E8C52602EBB2(3077.523f, 2069.8306f, -3.3524f, 3092.2957f, 2092.0422f, 2.769f, 0, 1);
		unk_0xC6AD633F6B896D88(3077.523f, 2069.8306f, -3.3524f, 3092.2957f, 2092.0422f, 2.769f, 0);
	}

	if (!unk_0x67670574D396B9A8(3062.67f, 2081.65f, 0f, 2f))
	{
		unk_0x8E09E8C52602EBB2(3054.367f, 2071.3608f, -2.4173f, 3075.6523f, 2091.9536f, 2.7296f, 0, 1);
		unk_0xC6AD633F6B896D88(3054.367f, 2071.3608f, -2.4173f, 3075.6523f, 2091.9536f, 2.7296f, 0);
	}

	if (!unk_0x67670574D396B9A8(3075.7163f, 2125.5903f, 0.8052f, 2f))
		unk_0x6FCA8E220FB1053D(3075.7163f, 2125.5903f, 0.8052f, 5f, 0);

	if (!unk_0x67670574D396B9A8(3070.8816f, 2103.9673f, -2.0883f, 2f))
		unk_0x10DFA2EC4C030EB3(3070.8816f, 2103.9673f, -2.0883f, 5f, 0, 0, 0, 0, 0, 0, 0);

	unk_0xDA95D0C19E737224("ENEMIES", &uLocal_171);
	unk_0x0133FF6E23A1DCA4(1, joaat("PLAYER"), joaat("PLAYER"));
	unk_0x0133FF6E23A1DCA4(3, joaat("PLAYER"), uLocal_171);
	unk_0x0133FF6E23A1DCA4(1, uLocal_171, uLocal_171);
	unk_0x0133FF6E23A1DCA4(3, uLocal_171, joaat("PLAYER"));
	unk_0x8386356641D0DED1(unk_0xC1A5EC5C986B98AD(), joaat("PLAYER"));
	_CONVERSATION_INITIALIZE_ACTOR(&uLocal_174, 1, unk_0xC1A5EC5C986B98AD(), "TREVOR", 0, 1);
	unk_0xFF2AF80F9EC7704C(0f);
	func_70(true);
	return;
}

void func_70(BOOL bParam0) // Position - 0x32AB
{
	func_71(39, bParam0);
	func_71(40, bParam0);
	func_71(41, bParam0);
	func_71(42, bParam0);
	func_71(43, bParam0);
	func_71(44, bParam0);
	return;
}

void func_71(int iParam0, BOOL bParam1) // Position - 0x32E3
{
	if (bParam1)
		if (!func_78(iParam0, 2, true))
			func_77(iParam0, 2, true);
	else if (func_78(iParam0, 2, true))
		func_72(iParam0, 2, true);

	return;
}

void func_72(int iParam0, int iParam1, BOOL bParam2) // Position - 0x331A
{
	int num;

	if (iParam0 == -1)
		return;

	if (bParam2)
	{
		unk_0x061B1200C95204CB(&Global_100733.f_1407[iParam0], iParam1);
	}
	else if (unk_0xA26A9A07F761D8F8())
	{
		if (func_10() == 0)
		{
			num = _MPCHAR_STAT_GET_INT(func_76(iParam0), -1, 0);
			unk_0x061B1200C95204CB(&num, iParam1);
			_MPCHAR_STAT_SET_INT(func_76(iParam0), num, -1, 1, false);
		}
	}
	else
	{
		unk_0x061B1200C95204CB(&Global_113648.f_668[iParam0], iParam1);
	}

	return;
}

void _MPCHAR_STAT_SET_INT(eMPStat empsParam0, int iParam1, int iParam2, int iParam3, BOOL bParam4) // Position - 0x338C
{
	int num;

	bParam4;
	num = Global_2805027[empsParam0 /*3*/][func_74(iParam2)];

	if (num != 0)
		unk_0x3CC35ACFFC9C730E(num, iParam1, iParam3);

	return;
}

int func_74(int iParam0) // Position - 0x33BC
{
	int num;
	int num2;

	num = iParam0;

	if (num == -1)
	{
		num2 = func_13();
	
		if (num2 > -1)
		{
			Global_2804739 = 0;
			num = num2;
		}
		else
		{
			num = 0;
			Global_2804739 = 1;
		}
	}

	return num;
}

int _MPCHAR_STAT_GET_INT(eMPStat empsParam0, int iParam1, int iParam2) // Position - 0x33F0
{
	var unk;
	int num;

	if (empsParam0 != 14192)
	{
		iParam2 == 0;
		unk = Global_2805027[empsParam0 /*3*/][func_74(iParam1)];
	
		if (unk_0xDD7756E2742E0F6D(unk, &num, -1))
			return num;
	}

	return 0;
}

eMPStat func_76(int iParam0) // Position - 0x342D
{
	switch (iParam0)
	{
		case 0:
			return MP_STAT_SHOPFM_HAIRDO_01_BH;
	
		case 1:
			return MP_STAT_SHOPFM_HAIRDO_02_SC;
	
		case 2:
			return MP_STAT_SHOPFM_HAIRDO_03_V;
	
		case 3:
			return MP_STAT_SHOPFM_HAIRDO_04_SS;
	
		case 4:
			return MP_STAT_SHOPFM_HAIRDO_05_MP;
	
		case 5:
			return MP_STAT_SHOPFM_HAIRDO_06_HW;
	
		case 6:
			return MP_STAT_SHOPFM_HAIRDO_07_PB;
	
		case 7:
			return MP_STAT_SHOPFM_CLOTHES_L_01_SC;
	
		case 8:
			return MP_STAT_SHOPFM_CLOTHES_L_02_GS;
	
		case 9:
			return MP_STAT_SHOPFM_CLOTHES_L_03_DT;
	
		case 10:
			return MP_STAT_SHOPFM_CLOTHES_L_04_CS;
	
		case 11:
			return MP_STAT_SHOPFM_CLOTHES_L_05_GSD;
	
		case 12:
			return MP_STAT_SHOPFM_CLOTHES_L_06_VC;
	
		case 13:
			return MP_STAT_SHOPFM_CLOTHES_L_07_PB;
	
		case 14:
			return MP_STAT_SHOPFM_CLOTHES_M_01_SM;
	
		case 15:
			return MP_STAT_SHOPFM_CLOTHES_M_03_H;
	
		case 16:
			return MP_STAT_SHOPFM_CLOTHES_M_04_HW;
	
		case 17:
			return MP_STAT_SHOPFM_CLOTHES_M_05_GOH;
	
		case 18:
			return MP_STAT_SHOPFM_CLOTHES_H_01_BH;
	
		case 19:
			return MP_STAT_SHOPFM_CLOTHES_H_02_B;
	
		case 20:
			return MP_STAT_SHOPFM_CLOTHES_H_03_MW;
	
		case 21:
			return MP_STAT_SHOPFM_CLOTHES_A_01_VB;
	
		case 22:
			return MP_STAT_SHOPFM_TATTOO_01_HW;
	
		case 23:
			return MP_STAT_SHOPFM_TATTOO_02_SS;
	
		case 24:
			return MP_STAT_SHOPFM_TATTOO_03_PB;
	
		case 25:
			return MP_STAT_SHOPFM_TATTOO_04_VC;
	
		case 26:
			return MP_STAT_SHOPFM_TATTOO_05_ELS;
	
		case 27:
			return MP_STAT_SHOPFM_TATTOO_06_GOH;
	
		case 28:
			return MP_STAT_SHOPFM_GUN_01_DT;
	
		case 29:
			return MP_STAT_SHOPFM_GUN_02_SS;
	
		case 30:
			return MP_STAT_SHOPFM_GUN_03_HW;
	
		case 31:
			return MP_STAT_SHOPFM_GUN_04_ELS;
	
		case 32:
			return MP_STAT_SHOPFM_GUN_05_PB;
	
		case 33:
			return MP_STAT_SHOPFM_GUN_06_LS;
	
		case 34:
			return MP_STAT_SHOPFM_GUN_07_MW;
	
		case 35:
			return MP_STAT_SHOPFM_GUN_08_CS;
	
		case 36:
			return MP_STAT_SHOPFM_GUN_09_GOH;
	
		case 37:
			return MP_STAT_SHOPFM_GUN_10_VWH;
	
		case 38:
			return MP_STAT_SHOPFM_GUN_11_ID1;
	
		case 39:
			return MP_STAT_SHOPFM_CARMOD_01_AP;
	
		case 40:
			return MP_STAT_SHOPFM_CARMOD_05_ID2;
	
		case 41:
			return MP_STAT_SHOPFM_CARMOD_06_BT1;
	
		case 42:
			return MP_STAT_SHOPFM_CARMOD_07_CS1;
	
		case 43:
			return MP_STAT_SHOPFM_CARMOD_08_CS6;
	
		case 44:
			return MP_STAT_RAYPISTOL_FM_AMMO_CURRENT;
	
		case 45:
			return MP_STAT_SHOPFM_PERS_GAR;
	
		case 46:
			return MP_STAT_SHOPFM_PERS_GUN;
	
		case 47:
			return MP_STAT_SHOPFM_PERS_GUN_OSPREY;
	
		case 48:
			return MP_STAT_SHOPFM_PERS_GUN_HTRUCK;
	
		case 49:
			return MP_STAT_SHOPFM_PERS_GUN_ARENA;
	
		case 52:
			return MP_STAT_SHOPFM_PERS_GUN_ARCADE;
	
		case 50:
			return MP_STAT_SHOPFM_HAIRDO_CASINO_APT;
	
		case 51:
			return MP_STAT_SHOPFM_CLOTHES_CASINO;
	
		case 53:
			return MP_STAT_SHOPFM_PERS_GUN_SUB;
	
		case 54:
			return MP_STAT_SHOPFM_TATTOO_07_CCT;
	
		case 55:
			return MP_STAT_SHOPFM_CLOTHES_CAR_MEET;
	
		case 56:
			return MP_STAT_SHOPFM_PERS_GUN_FIXER;
	
		case 57:
			return MP_STAT_SHOPFM_CLOTHES_STUDIO;
	
		case 58:
			return MP_STAT_GBTELTIMESPLAYEDGOONPREV;
	
		case 59:
			return MP_STAT_TAMPA3_FMINI_HELDTIME;
	
		default:
			break;
	}

	return 14192;
}

void func_77(int iParam0, int iParam1, BOOL bParam2) // Position - 0x37C6
{
	int num;

	if (iParam0 == -1)
		return;

	if (bParam2)
	{
		unk_0xECDAB41968FF21A8(&Global_100733.f_1407[iParam0], iParam1);
	}
	else if (unk_0xA26A9A07F761D8F8())
	{
		if (func_10() == 0)
		{
			num = _MPCHAR_STAT_GET_INT(func_76(iParam0), -1, 0);
			unk_0xECDAB41968FF21A8(&num, iParam1);
			_MPCHAR_STAT_SET_INT(func_76(iParam0), num, -1, 1, false);
		}
	}
	else
	{
		unk_0xECDAB41968FF21A8(&Global_113648.f_668[iParam0], iParam1);
	}

	return;
}

BOOL func_78(int iParam0, int iParam1, BOOL bParam2) // Position - 0x3838
{
	if (iParam0 == -1)
		return false;

	if (bParam2)
		return IS_BIT_SET(Global_100733.f_1407[iParam0], iParam1);
	else if (unk_0xA26A9A07F761D8F8())
		if (func_10() == 0)
			return IS_BIT_SET(_MPCHAR_STAT_GET_INT(func_76(iParam0), -1, 0), iParam1);
	else
		return IS_BIT_SET(Global_113648.f_668[iParam0], iParam1);

	return false;
}

void _CONVERSATION_INITIALIZE_ACTOR(var uParam0, int iParam1, var uParam2, char* sParam3, int iParam4, int iParam5) // Position - 0x3899
{
	uParam0->[iParam1 /*10*/].f_7 == 1;
	uParam0->[iParam1 /*10*/] = uParam2;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->[iParam1 /*10*/].f_1), sParam3, 24);
	uParam0->[iParam1 /*10*/].f_7 = 1;
	uParam0->[iParam1 /*10*/].f_8 = iParam4;
	uParam0->[iParam1 /*10*/].f_9 = iParam5;

	if (!Global_78558)
	{
		if (!unk_0x66599E73DBA5A850(uParam2))
			if (uParam0->[iParam1 /*10*/].f_8 == 0)
				unk_0xAD4CEC14FE6404F6(uParam2, 0);
			else
				unk_0xAD4CEC14FE6404F6(uParam2, 1);
	
		if (!unk_0x66599E73DBA5A850(uParam2))
			if (uParam0->[iParam1 /*10*/].f_9 == 0)
				unk_0x59B8D6E436F5B21C(uParam2, 0);
			else
				unk_0x59B8D6E436F5B21C(uParam2, 1);
	}

	return;
}

void func_80(int iParam0) // Position - 0x3934
{
	switch (iParam0)
	{
		case 0:
			uLocal_172[iParam0] = unk_0x54ECDA2DAB9CFB10(3054.991f, 1995.8871f, -20f, 3140.8171f, 2225.639f, 10.453f, 0, 1, 1, 1);
			break;
	}

	return;
}

void _DEBUG_PRINT(int iParam0) // Position - 0x3978
{
	iParam0 = iParam0;
	return;
}

void func_82() // Position - 0x3984
{
	if (!func_120())
		if (func_107())
			func_169(true, false);

	unk_0x385C3C9C52E9D548("RE_SEAPLANE", 0);
	func_106();
	func_94();
	func_90();
	func_85();
	func_83(&uLocal_158, &uLocal_150);
	return;
}

void func_83(var uParam0, var uParam1) // Position - 0x39C1
{
	int num;
	int i;

	if (uParam0->[*uParam1 /*6*/] != 0 && unk_0x7DE17ACDD8BA2642(uParam0->[*uParam1 /*6*/]) && uParam0->[*uParam1 /*6*/].f_1 != 0 && unk_0x7DE17ACDD8BA2642(uParam0->[*uParam1 /*6*/].f_1) && uParam0->[*uParam1 /*6*/].f_4)
		if (func_84(uParam0->[*uParam1 /*6*/].f_2, 2))
			uParam0->[*uParam1 /*6*/].f_5 = unk_0xA1DF058198143448(uParam0->[*uParam1 /*6*/], uParam0->[*uParam1 /*6*/].f_1);
		else
			uParam0->[*uParam1 /*6*/].f_5 = unk_0xC832FC847E3442DA(uParam0->[*uParam1 /*6*/], uParam0->[*uParam1 /*6*/].f_1, uParam0->[*uParam1 /*6*/].f_3);

	num = num + 1;
	*uParam1 = *uParam1 + 1;

	if (*uParam1 >= *uParam0)
		*uParam1 = 0;

	while (uParam0->[*uParam1 /*6*/] == 0 || !unk_0x7DE17ACDD8BA2642(uParam0->[*uParam1 /*6*/]) || uParam0->[*uParam1 /*6*/].f_1 == 0 || !unk_0x7DE17ACDD8BA2642(uParam0->[*uParam1 /*6*/].f_1) && num < *uParam0)
	{
		num = num + 1;
		*uParam1 = *uParam1 + 1;
	
		if (*uParam1 >= *uParam0)
			*uParam1 = 0;
	}

	for (i = 0; i < *uParam0; i = i + 1)
	{
		if (!uParam0->[i /*6*/].f_4)
		{
			uParam0->[i /*6*/] = 0;
			uParam0->[i /*6*/].f_1 = 0;
			uParam0->[i /*6*/].f_2 = 0;
			uParam0->[i /*6*/].f_3 = 0;
			uParam0->[i /*6*/].f_5 = 0;
		}
	
		uParam0->[i /*6*/].f_4 = 0;
	}

	return;
}

BOOL func_84(int iParam0, int iParam1) // Position - 0x3B47
{
	return iParam0 && iParam1 != false;
}

void func_85() // Position - 0x3B56
{
	int i;

	for (i = 0; i < 11; i = i + 1)
	{
		if (unk_0x7DE17ACDD8BA2642(uLocal_49[i /*6*/]) && unk_0xCE4AAA035282336C(uLocal_49[i /*6*/].f_1))
		{
			if (unk_0x3C3D6D026CF7F51B(uLocal_49[i /*6*/], 0))
			{
				if (!func_89(i, 0))
				{
					unk_0x225EB85DBC38E707(uLocal_49[i /*6*/].f_1, 1f);
					func_58(i, 0);
				}
			
				if (_GET_VEHICLE_SEAT_PED_IS_IN(uLocal_49[i /*6*/], 0) != -1)
				{
					if (func_87(i))
					{
						if (unk_0xB77B29436394B1EB(uLocal_49[i /*6*/].f_1) > 0)
						{
							unk_0x5E3A54CF3E9DF9FC(uLocal_49[i /*6*/].f_1, 0);
							unk_0xDE9F0903D46A94EF(uLocal_49[i /*6*/].f_1, 1);
						}
					}
					else if (unk_0xB77B29436394B1EB(uLocal_49[i /*6*/].f_1) == 0)
					{
						unk_0x5E3A54CF3E9DF9FC(uLocal_49[i /*6*/].f_1, 255);
						unk_0xDE9F0903D46A94EF(uLocal_49[i /*6*/].f_1, 0);
					}
				}
			}
			else if (func_89(i, 0))
			{
				unk_0x225EB85DBC38E707(uLocal_49[i /*6*/].f_1, 0.7f);
				func_86(i, 0);
			
				if (unk_0xB77B29436394B1EB(uLocal_49[i /*6*/].f_1) == 0)
				{
					unk_0x5E3A54CF3E9DF9FC(uLocal_49[i /*6*/].f_1, 255);
					unk_0xDE9F0903D46A94EF(uLocal_49[i /*6*/].f_1, 0);
				}
			}
		
			if (unk_0x66599E73DBA5A850(uLocal_49[i /*6*/]))
				func_43(&(uLocal_49[i /*6*/].f_1));
		}
	}

	for (i = 0; i < 5; i = i + 1)
	{
		if (unk_0x7DE17ACDD8BA2642(uLocal_116[i /*4*/]) && unk_0xCE4AAA035282336C(uLocal_116[i /*4*/].f_1))
			if (!unk_0xFBD273FDBCF0C5BD(uLocal_116[i /*4*/], 0))
				func_43(&(uLocal_116[i /*4*/].f_1));
	}

	return;
}

void func_86(int iParam0, int iParam1) // Position - 0x3CE0
{
	unk_0x061B1200C95204CB(&(uLocal_49[iParam0 /*6*/].f_2), iParam1);
	return;
}

BOOL func_87(int iParam0) // Position - 0x3CF6
{
	if (iParam0 == 1)
		if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[0 /*6*/]))
			return false;

	if (iParam0 == 3)
		if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[2 /*6*/]))
			return false;

	if (iParam0 == 4)
		if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[2 /*6*/]) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[3 /*6*/]))
			return false;

	if (iParam0 == 8)
		if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[7 /*6*/]))
			return false;

	if (iParam0 == 10)
		if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[9 /*6*/]))
			return false;

	return true;
}

int _GET_VEHICLE_SEAT_PED_IS_IN(var ped, int includeLastVehicle) // Position - 0x3D84
{
	var unk;
	int num;
	int i;
	int num2;

	if (!unk_0x66599E73DBA5A850(ped))
	{
		if (unk_0x3C3D6D026CF7F51B(ped, includeLastVehicle))
		{
			unk = unk_0x95DC39736F6748E3(ped, includeLastVehicle);
		
			if (unk_0x7DE17ACDD8BA2642(unk))
			{
				num = unk_0xB47599824058F1C1(unk_0x504B9BB48D41C264(unk));
			
				if (num == 1)
				{
					num2 = -1;
					return num2;
				}
			
				for (i = 0; i < num; i = i + 1)
				{
					num2 = i - 1;
				
					if (!unk_0x9CFCDD9C62B56708(unk, num2, 0))
						if (unk_0xA66E176E5772E965(unk, num2, 0) == ped)
							return num2;
				}
			}
		}
	}

	return num2;
}

BOOL func_89(int iParam0, int iParam1) // Position - 0x3E09
{
	return IS_BIT_SET(uLocal_49[iParam0 /*6*/].f_2, iParam1);
}

void func_90() // Position - 0x3E1C
{
	int i;

	for (i = 0; i < 5; i = i + 1)
	{
		if (unk_0x7DE17ACDD8BA2642(uLocal_116[i /*4*/]))
			func_91(i);
	}

	return;
}

void func_91(int iParam0) // Position - 0x3E49
{
	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[iParam0 /*4*/]) || iParam0 == 0)
	{
		switch (iParam0)
		{
			case 1:
			case 2:
			case 3:
			case 4:
				switch (uLocal_116[iParam0 /*4*/].f_2)
				{
					case 0:
						if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[iParam0 /*4*/], -1, 0)) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[iParam0 /*4*/], 0, 0)) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[iParam0 /*4*/], 1, 0)) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[iParam0 /*4*/], 2, 0)))
						{
							unk_0x53A563E258997DA8(uLocal_116[iParam0 /*4*/], 1);
							unk_0xA1FC89AFB27CD4A4(uLocal_116[iParam0 /*4*/], 0);
							uLocal_116[iParam0 /*4*/].f_2 = uLocal_116[iParam0 /*4*/].f_2 + 1;
						}
						break;
				
					case 1:
						break;
				}
				break;
		}
	}
	else if (unk_0x51DF6895C236B231(uLocal_116[iParam0 /*4*/]))
	{
		func_92(iParam0, false);
	}
	else
	{
		func_92(iParam0, true);
	}

	return;
}

void func_92(int iParam0, BOOL bParam1) // Position - 0x3F48
{
	if (unk_0x7DE17ACDD8BA2642(uLocal_116[iParam0 /*4*/]))
	{
		func_43(&(uLocal_116[iParam0 /*4*/].f_1));
	
		if (unk_0xA005AFE85A66532E(uLocal_116[iParam0 /*4*/]))
			unk_0xA470B8FA40799169(uLocal_116[iParam0 /*4*/]);
	
		if (func_93(&uLocal_116[iParam0 /*4*/]))
			if (bParam1)
				unk_0x2728CF7242F08BD5(&uLocal_116[iParam0 /*4*/]);
			else
				unk_0x0E4B6CF706BE8AA4(&uLocal_116[iParam0 /*4*/]);
	
		uLocal_116[iParam0 /*4*/] = 0;
		uLocal_116[iParam0 /*4*/].f_2 = 0;
		uLocal_116[iParam0 /*4*/].f_3 = 0f;
	}

	return;
}

BOOL func_93(var uParam0) // Position - 0x3FC2
{
	if (unk_0xE0D346789C5160EB(unk_0xC1A5EC5C986B98AD(), *uParam0, 0))
		return false;

	return true;
}

void func_94() // Position - 0x3FDE
{
	int i;

	for (i = 0; i < 11; i = i + 1)
	{
		if (unk_0x7DE17ACDD8BA2642(uLocal_49[i /*6*/]))
			func_95(i);
	}

	return;
}

void func_95(int iParam0) // Position - 0x400C
{
	BOOL flag;
	int num;
	int num2;
	int num3;
	char* str;
	int num4;
	var unk;
	int num5;
	float num6;
	float num7;

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[iParam0 /*6*/]))
	{
		switch (iParam0)
		{
			case 0:
			case 2:
				if (uLocal_49[iParam0 /*6*/].f_3 >= 2 && uLocal_49[iParam0 /*6*/].f_3 < 5)
				{
					if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)) && unk_0x584CD220C093B0B4(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
					{
						func_105();
						unk_0x82C6DD5E8460E2C0(0, 1);
						unk_0x6FE9A0C01D25F413(0, unk_0x15A88CFAAFFC6C4B(500, 1501), 4096);
						unk_0x82C6DD5E8460E2C0(0, 0);
						unk_0xCAC2B0C65B18E755(0, unk_0xC1A5EC5C986B98AD(), 0, 16);
						func_103(uLocal_49[iParam0 /*6*/]);
						unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
						uLocal_49[iParam0 /*6*/].f_3 = 5;
					}
				}
			
				switch (uLocal_49[iParam0 /*6*/].f_3)
				{
					case 0:
						if (iParam0 == 0)
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[3 /*6*/]))
								unk_0xB96518A6F91FC977(uLocal_49[iParam0 /*6*/], 1, uLocal_49[3 /*6*/], 31086, uLocal_139, 0, -1, -1);
						else if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[1 /*6*/]))
							unk_0xB96518A6F91FC977(uLocal_49[iParam0 /*6*/], 1, uLocal_49[1 /*6*/], 31086, uLocal_139, 0, -1, -1);
					
						if (func_121(1) || unk_0xB8C39D031C377E33(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD()) || func_102() || func_101())
						{
							func_100(iParam0);
							unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 1:
						if (iParam0 == 0)
							if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[1 /*6*/]) || _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[1 /*4*/]) && unk_0xE0D346789C5160EB(uLocal_49[1 /*6*/], uLocal_116[1 /*4*/], 1))
								flag = true;
						else if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[3 /*6*/]) || _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[2 /*4*/]) && unk_0xE0D346789C5160EB(uLocal_49[3 /*6*/], uLocal_116[2 /*4*/], 1) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[4 /*6*/]) || _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[2 /*4*/]) && unk_0xE0D346789C5160EB(uLocal_49[4 /*6*/], uLocal_116[2 /*4*/], 1))
							flag = true;
					
						if (flag)
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						break;
				
					case 2:
						num4 = 526;
					
						if (iParam0 == 0)
							str = sLocal_137;
						else
							str = sLocal_138;
					
						unk_0x3B0DF1BD4F04502A(str, unk_0x30BE8A934C020461(unk_0xC1A5EC5C986B98AD(), 1), &num);
						unk_0x091A46DB05F8242A(str, &num3);
						unk_0x3B0DF1BD4F04502A(str, unk_0x30BE8A934C020461(uLocal_49[iParam0 /*6*/], 1), &num2);
					
						if (num > 0 && unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()))
							num = num - 1;
					
						if (iParam0 == 2 && !unk_0x9DC9E896F189AAA5(unk_0xC1A5EC5C986B98AD(), 3108.6753f, 2194.1206f, 10.950867f, 3143.5254f, 2232.2651f, -4.598444f, 82.75f, 0, 1, 0))
							num = func_99(num, 22, num3);
					
						if (func_38(unk_0xC1A5EC5C986B98AD(), uLocal_145, 1) >= 225f)
							num = num3 - 1;
					
						if (iParam0 == 0 && iLocal_151 != num && num > num2 || iParam0 == 2 && iLocal_152 != num && num > num2 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])) && !unk_0xED74DF0E946E0A76(uLocal_49[iParam0 /*6*/], 0))
						{
							if (iParam0 == 0)
								iLocal_151 = num;
							else
								iLocal_152 = num;
						
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])))
							{
								unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0);
								func_86(iParam0, 5);
							}
						
							unk_0x90003857B628DA1E(uLocal_49[iParam0 /*6*/], unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), str, 786469, 0, num4, num, -1f, 0, 6f);
						}
					
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_FOLLOW_WAYPOINT_RECORDING"), true) && unk_0xF5F493B789EA063E(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_FOLLOW_WAYPOINT_RECORDING")) == 7 && !func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_SHOOT_AT_ENTITY"), true))
						{
							unk_0x180D859471330484(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 1101004800);
						
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])) && unk_0x74D15F81528C1714(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])))
							{
								unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 1);
								func_58(iParam0, 5);
							}
						}
					
						if (!func_89(iParam0, 4))
						{
							if (unk_0xED74DF0E946E0A76(uLocal_49[iParam0 /*6*/], 0))
							{
								func_58(iParam0, 4);
								unk_0x909F139DC199D8E0(uLocal_49[iParam0 /*6*/]);
							
								if (!func_89(iParam0, 5) && unk_0x74D15F81528C1714(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])))
									unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 1);
							}
						}
						else if (!unk_0xED74DF0E946E0A76(uLocal_49[iParam0 /*6*/], 0))
						{
							func_86(iParam0, 4);
						
							if (!func_89(iParam0, 5))
								unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0);
						}
					
						if (unk_0xF5F493B789EA063E(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_FOLLOW_WAYPOINT_RECORDING")) == 1 && !unk_0xF193A4B306E6BF93(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/])) && !unk_0x654D26EF23C82D70(uLocal_49[iParam0 /*6*/]))
							unk_0x6855D2807847D85A(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, uLocal_139, 500f, 100, 1, joaat("FIRING_PATTERN_BURST_FIRE"));
					
						unk_0x3A1F6C7E7B1FC147(str, num3 - 1, &unk);
					
						if (unk_0x1C4E4DC1EFE24DF1(uLocal_49[iParam0 /*6*/], unk, 8f, 8f, 8f, 0, 1, 0))
						{
							unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							unk_0xE0A549732EBB1749(uLocal_49[iParam0 /*6*/], unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0, unk_0xC1A5EC5C986B98AD(), uLocal_139, 7, 20f, 786469, 5f, 7);
							func_68(5);
							unk_0x0DE8FEF68764819C(str);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 3:
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
						{
							if (unk_0x248929CB6622481D(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), 3, 6000) || func_97(iParam0, 5000))
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
								unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
								uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
							}
							else if (unk_0xF5F493B789EA063E(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_MISSION")) == 1 && !unk_0x654D26EF23C82D70(uLocal_49[iParam0 /*6*/]))
							{
								unk_0x6855D2807847D85A(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, uLocal_139, 500f, 100, 1, joaat("FIRING_PATTERN_BURST_FIRE"));
							}
						}
						break;
				
					case 4:
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_GET_OFF_BOAT"), true))
						{
							if (!unk_0xE03DB5D994E334C8(uLocal_49[iParam0 /*6*/]))
							{
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
								unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							}
							else
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
							}
						}
						break;
				}
				break;
		
			case 1:
			case 3:
			case 4:
				switch (uLocal_49[iParam0 /*6*/].f_3)
				{
					case 0:
						if (iParam0 == 1 && func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_START_SCENARIO_IN_PLACE"), true) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[3 /*6*/]))
							unk_0xB96518A6F91FC977(uLocal_49[iParam0 /*6*/], 1, uLocal_49[3 /*6*/], 31086, uLocal_139, 0, -1, -1);
					
						if (iParam0 == 3 || iParam0 == 4 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_49[1 /*6*/]) && func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_START_SCENARIO_IN_PLACE"), true))
							unk_0xB96518A6F91FC977(uLocal_49[iParam0 /*6*/], 1, uLocal_49[1 /*6*/], 31086, uLocal_139, 0, -1, -1);
					
						if (func_121(1) || unk_0xB8C39D031C377E33(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD()) || func_102() || func_101())
						{
							func_100(iParam0);
							uLocal_49[iParam0 /*6*/].f_4 = unk_0xA5E11AF0A2B928C1() + unk_0x15A88CFAAFFC6C4B(1250, 1501);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 1:
						if (unk_0xA5E11AF0A2B928C1() >= uLocal_49[iParam0 /*6*/].f_4)
						{
							if (iParam0 == 1 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[1 /*4*/]) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[1 /*4*/], -1, 0)))
							{
								unk_0x909F139DC199D8E0(uLocal_49[iParam0 /*6*/]);
								unk_0xD30E9CAE1FEA1C7E(uLocal_49[iParam0 /*6*/], uLocal_116[1 /*4*/], 20000, 0, 3f, 1, 0);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							}
						
							if (iParam0 == 3 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[2 /*4*/]) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[2 /*4*/], -1, 0)))
							{
								unk_0x909F139DC199D8E0(uLocal_49[iParam0 /*6*/]);
								unk_0xD30E9CAE1FEA1C7E(uLocal_49[iParam0 /*6*/], uLocal_116[2 /*4*/], 20000, 0, 3f, 1, 0);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							}
						
							if (iParam0 == 4 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[2 /*4*/]) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[2 /*4*/], -1, 0)))
							{
								unk_0x909F139DC199D8E0(uLocal_49[iParam0 /*6*/]);
								unk_0xD30E9CAE1FEA1C7E(uLocal_49[iParam0 /*6*/], uLocal_116[2 /*4*/], 20000, 1, 3f, 1, 0);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							}
						
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 2:
						switch (iParam0)
						{
							case 1:
								num5 = 1;
								break;
						
							case 3:
								num5 = 2;
								break;
						
							case 4:
								num5 = 2;
								break;
						}
					
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 1) || !_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(uLocal_116[num5 /*4*/], -1, 0)))
						{
							unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
							unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						else if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_ENTER_VEHICLE"), true))
						{
							if (iParam0 == 1 || iParam0 == 3 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[num5 /*4*/]))
								unk_0xD30E9CAE1FEA1C7E(uLocal_49[iParam0 /*6*/], uLocal_116[num5 /*4*/], 20000, 0, 3f, 1, 0);
						
							if (iParam0 == 4 && _DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[num5 /*4*/]))
								unk_0xD30E9CAE1FEA1C7E(uLocal_49[iParam0 /*6*/], uLocal_116[num5 /*4*/], 20000, 1, 3f, 1, 0);
						}
						break;
				
					case 3:
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
						{
							if (unk_0x584CD220C093B0B4(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
							{
								func_105();
								unk_0x82C6DD5E8460E2C0(0, 1);
								unk_0x6FE9A0C01D25F413(0, unk_0x15A88CFAAFFC6C4B(500, 1501), 4096);
								unk_0x82C6DD5E8460E2C0(0, 0);
								unk_0xCAC2B0C65B18E755(0, unk_0xC1A5EC5C986B98AD(), 0, 16);
								func_103(uLocal_49[iParam0 /*6*/]);
								unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
								uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
							}
						
							if (uLocal_49[iParam0 /*6*/].f_3 == 3)
							{
								if (unk_0x248929CB6622481D(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), 3, 6000) || func_97(iParam0, 5000) && func_121(5))
								{
									unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], 20000);
									unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
									unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
									uLocal_49[iParam0 /*6*/].f_3 = 5;
								}
								else if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), -1, 0)))
								{
									unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 3, 1);
									unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
									uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
								}
							}
						}
						break;
				
					case 4:
						break;
				
					case 5:
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_GET_OFF_BOAT"), true))
						{
							if (!unk_0xE03DB5D994E334C8(uLocal_49[iParam0 /*6*/]))
							{
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
								unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							}
							else
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
							}
						}
						break;
				}
				break;
		
			case 7:
				if (uLocal_49[iParam0 /*6*/].f_3 >= 1 && uLocal_49[iParam0 /*6*/].f_3 < 3)
				{
					if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)) && unk_0x584CD220C093B0B4(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
					{
						func_105();
						unk_0x82C6DD5E8460E2C0(0, 1);
						unk_0x6FE9A0C01D25F413(0, unk_0x15A88CFAAFFC6C4B(500, 1501), 4096);
						unk_0x82C6DD5E8460E2C0(0, 0);
						unk_0xCAC2B0C65B18E755(0, unk_0xC1A5EC5C986B98AD(), 0, 16);
						func_103(uLocal_49[iParam0 /*6*/]);
						unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
						uLocal_49[iParam0 /*6*/].f_3 = 3;
					}
				}
			
				switch (uLocal_49[iParam0 /*6*/].f_3)
				{
					case 0:
						if (func_44() && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 125f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0x1589BC95A4C50F21(unk_0xC1A5EC5C986B98AD()) && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 150f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0xDB08595445CDE2FF(uLocal_49[iParam0 /*6*/]) || func_121(6))
						{
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[3 /*4*/]))
							{
								unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0);
								num6 = unk_0xDC58AE028CB223BA(unk_0xC1A5EC5C986B98AD());
								num6 = func_96(num6, 10f, 40f);
								unk_0xC75D5CC27D850469(uLocal_116[3 /*4*/], num6);
							}
						
							unk_0x8CC688927B10006C(uLocal_49[iParam0 /*6*/], joaat("WEAPON_PISTOL"), 0);
							unk_0xE0A549732EBB1749(uLocal_49[iParam0 /*6*/], unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0, unk_0xC1A5EC5C986B98AD(), uLocal_139, 7, 20f, 786469, 5f, 7);
							unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], true, 145);
							func_68(6);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 1:
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
						{
							if (unk_0x248929CB6622481D(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), 3, 6000) || func_97(iParam0, 5000))
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
								unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
								uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
							}
							else if (unk_0xF5F493B789EA063E(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_MISSION")) == 1 && !unk_0x654D26EF23C82D70(uLocal_49[iParam0 /*6*/]))
							{
								unk_0x6855D2807847D85A(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, uLocal_139, 500f, 100, 1, joaat("FIRING_PATTERN_BURST_FIRE"));
							}
						}
						break;
				
					case 2:
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_GET_OFF_BOAT"), true))
						{
							if (!unk_0xE03DB5D994E334C8(uLocal_49[iParam0 /*6*/]))
							{
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
								unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							}
							else
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
							}
						}
						break;
				}
				break;
		
			case 9:
				if (uLocal_49[iParam0 /*6*/].f_3 >= 1 && uLocal_49[iParam0 /*6*/].f_3 < 3)
				{
					if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)) && unk_0x584CD220C093B0B4(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
					{
						func_105();
						unk_0x82C6DD5E8460E2C0(0, 1);
						unk_0x6FE9A0C01D25F413(0, unk_0x15A88CFAAFFC6C4B(500, 1501), 4096);
						unk_0x82C6DD5E8460E2C0(0, 0);
						unk_0xCAC2B0C65B18E755(0, unk_0xC1A5EC5C986B98AD(), 0, 16);
						func_103(uLocal_49[iParam0 /*6*/]);
						unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
						uLocal_49[iParam0 /*6*/].f_3 = 3;
					}
				}
			
				switch (uLocal_49[iParam0 /*6*/].f_3)
				{
					case 0:
						if (func_44() && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 125f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0x1589BC95A4C50F21(unk_0xC1A5EC5C986B98AD()) && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 150f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0xDB08595445CDE2FF(uLocal_49[iParam0 /*6*/]) || func_121(6))
						{
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[4 /*4*/]))
							{
								unk_0xA1FC89AFB27CD4A4(unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0);
								num7 = unk_0xDC58AE028CB223BA(unk_0xC1A5EC5C986B98AD());
								num7 = func_96(num7, 10f, 40f);
								unk_0xC75D5CC27D850469(uLocal_116[4 /*4*/], num7);
							}
						
							unk_0x8CC688927B10006C(uLocal_49[iParam0 /*6*/], joaat("WEAPON_PISTOL"), 0);
							unk_0xE0A549732EBB1749(uLocal_49[iParam0 /*6*/], unk_0xB3011BC7288F3091(uLocal_49[iParam0 /*6*/]), 0, unk_0xC1A5EC5C986B98AD(), uLocal_139, 7, 20f, 786469, 5f, 7);
							unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
							uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], true, 145);
							func_68(6);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 1:
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
						{
							if (unk_0x248929CB6622481D(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), 3, 6000) || func_97(iParam0, 5000))
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
								unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
								uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
							}
							else if (unk_0xF5F493B789EA063E(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_VEHICLE_MISSION")) == 1 && !unk_0x654D26EF23C82D70(uLocal_49[iParam0 /*6*/]))
							{
								unk_0x6855D2807847D85A(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, uLocal_139, 500f, 100, 1, joaat("FIRING_PATTERN_BURST_FIRE"));
							}
						}
						break;
				
					case 2:
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_GET_OFF_BOAT"), true))
						{
							if (!unk_0xE03DB5D994E334C8(uLocal_49[iParam0 /*6*/]))
							{
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
								unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							}
							else
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
							}
						}
						break;
				}
				break;
		
			case 8:
			case 10:
				if (uLocal_49[iParam0 /*6*/].f_3 >= 1 && uLocal_49[iParam0 /*6*/].f_3 < 3)
				{
					if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0) && _DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)) && unk_0x584CD220C093B0B4(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
					{
						func_105();
						unk_0x82C6DD5E8460E2C0(0, 1);
						unk_0x6FE9A0C01D25F413(0, unk_0x15A88CFAAFFC6C4B(500, 1501), 4096);
						unk_0x82C6DD5E8460E2C0(0, 0);
						unk_0xCAC2B0C65B18E755(0, unk_0xC1A5EC5C986B98AD(), 0, 16);
						func_103(uLocal_49[iParam0 /*6*/]);
						unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
						uLocal_49[iParam0 /*6*/].f_3 = 3;
					}
				}
			
				switch (uLocal_49[iParam0 /*6*/].f_3)
				{
					case 0:
						if (func_44() && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 125f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0x1589BC95A4C50F21(unk_0xC1A5EC5C986B98AD()) && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_49[iParam0 /*6*/], 1) <= 150f || unk_0x4FBDBA15B9C41492(unk_0xC1A5EC5C986B98AD()) && !unk_0xDB08595445CDE2FF(uLocal_49[iParam0 /*6*/]))
						{
							uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], true, 145);
							unk_0x5E3A54CF3E9DF9FC(uLocal_49[iParam0 /*6*/].f_1, 0);
							unk_0xDE9F0903D46A94EF(uLocal_49[iParam0 /*6*/].f_1, 1);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 1:
						if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0))
						{
							if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0)))
							{
								if (unk_0x248929CB6622481D(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), 3, 6000) || func_97(iParam0, 5000))
								{
									unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
									unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
									unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 1, 0);
									uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
								}
								else if (!_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xA66E176E5772E965(unk_0x95DC39736F6748E3(uLocal_49[iParam0 /*6*/], 0), -1, 0)))
								{
									unk_0x7FE5E05BE63F6CB8(uLocal_49[iParam0 /*6*/], 3, 1);
									unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
									uLocal_49[iParam0 /*6*/].f_3 = 3;
								}
							}
						}
						else
						{
							unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							uLocal_49[iParam0 /*6*/].f_3 = uLocal_49[iParam0 /*6*/].f_3 + 1;
						}
						break;
				
					case 2:
						if (!func_98(uLocal_49[iParam0 /*6*/], joaat("SCRIPT_TASK_GET_OFF_BOAT"), true))
						{
							if (!unk_0xE03DB5D994E334C8(uLocal_49[iParam0 /*6*/]))
							{
								unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 0);
								unk_0xCAC2B0C65B18E755(uLocal_49[iParam0 /*6*/], unk_0xC1A5EC5C986B98AD(), 0, 16);
							}
							else
							{
								unk_0xFF5DB4600BC807C7(uLocal_49[iParam0 /*6*/], -1);
							}
						}
						break;
				}
				break;
		}
	}
	else
	{
		if (func_89(iParam0, 1))
		{
			unk_0xAC2C285C82A9244C(uLocal_49[iParam0 /*6*/], 1);
			unk_0xA81AA70A4D25E140(uLocal_49[iParam0 /*6*/], 1, 0);
			func_86(iParam0, 1);
		}
	
		if (!func_89(iParam0, 2))
			if (unk_0x51DF6895C236B231(uLocal_49[iParam0 /*6*/]))
				func_40(iParam0, false);
			else
				func_40(iParam0, true);
	}

	return;
}

float func_96(float fParam0, float fParam1, float fParam2) // Position - 0x5502
{
	if (fParam0 > fParam2)
		return fParam2;
	else if (fParam0 < fParam1)
		return fParam1;

	return fParam0;
}

int func_97(int iParam0, int iParam1) // Position - 0x5529
{
	int num;

	switch (iParam0)
	{
		case 0:
		case 1:
			num = 1;
			break;
	
		case 2:
		case 3:
		case 4:
			num = 2;
			break;
	
		case 7:
		case 8:
			num = 3;
			break;
	
		case 9:
		case 10:
			num = 4;
			break;
	}

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[num /*4*/]))
		if (!unk_0x1589BC95A4C50F21(uLocal_116[num /*4*/]))
			uLocal_116[num /*4*/].f_3 = uLocal_116[num /*4*/].f_3 + unk_0x290D4E218346D595();
		else
			uLocal_116[num /*4*/].f_3 = 0f;

	if (uLocal_116[num /*4*/].f_3 >= (float)iParam1)
		return 1;

	return 0;
}

BOOL func_98(var uParam0, int iParam1, BOOL bParam2) // Position - 0x55D9
{
	int num;

	num = unk_0xF5F493B789EA063E(uParam0, iParam1);

	if (num == 1 || num == 0)
		return true;
	else if (!bParam2)
		if (num == 2 || num == 3)
			return true;

	return false;
}

int func_99(int iParam0, int iParam1, int iParam2) // Position - 0x561F
{
	if (iParam0 > iParam2)
		return iParam2;
	else if (iParam0 < iParam1)
		return iParam1;

	return iParam0;
}

void func_100(int iParam0) // Position - 0x5644
{
	func_58(iParam0, 3);

	if (!func_121(4))
		func_68(4);

	if (!func_121(2))
		func_68(2);

	uLocal_49[iParam0 /*6*/].f_1 = func_60(uLocal_49[iParam0 /*6*/], true, 145);
	unk_0xEA1F0CBFFACFA528(uLocal_49[iParam0 /*6*/], 200f);

	if (unk_0x3C3D6D026CF7F51B(uLocal_49[iParam0 /*6*/], 0))
	{
		if (_GET_VEHICLE_SEAT_PED_IS_IN(uLocal_49[iParam0 /*6*/], 0) != -1)
		{
			unk_0x5E3A54CF3E9DF9FC(uLocal_49[iParam0 /*6*/].f_1, 0);
			unk_0xDE9F0903D46A94EF(uLocal_49[iParam0 /*6*/].f_1, 1);
		}
		else
		{
			unk_0x909F139DC199D8E0(uLocal_49[iParam0 /*6*/]);
			unk_0xC5B2830898581862(uLocal_49[iParam0 /*6*/], 1);
		}
	}

	return;
}

BOOL func_101() // Position - 0x56E2
{
	return unk_0xAFC34601EB143C10(-1, 3150.5076f, 2179.3433f, -30f, 3064.6077f, 2223.616f, 12.381769f, 103f);
}

BOOL func_102() // Position - 0x5712
{
	var unk;

	unk = unk_0xE475C458F52F1781();

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk) && unk_0x9DC9E896F189AAA5(unk, 3083.2302f, 2195.5156f, -3.22382f, 3096.5188f, 2195.4553f, 4.949678f, 15f, 0, 1, 0))
		return true;

	return false;
}

void func_103(var uParam0) // Position - 0x575F
{
	if (func_121(0))
	{
		unk_0xF2CFC6EC8C85FA78(uLocal_339);
		func_104(0);
	}

	unk_0x3D7110D966B0CEA2(uParam0, uLocal_339);
	unk_0x63EF69C6969A3D26(&uLocal_339);
	return;
}

void func_104(int iParam0) // Position - 0x578B
{
	unk_0x061B1200C95204CB(&iLocal_149, iParam0);
	return;
}

void func_105() // Position - 0x579B
{
	if (!func_121(0))
	{
		unk_0x63EF69C6969A3D26(&uLocal_339);
		unk_0xDBC7406226B5540E(&uLocal_339);
		func_68(0);
	}

	return;
}

void func_106() // Position - 0x57BF
{
	BOOL flag;
	int i;

	if (func_121(1) || func_121(4))
	{
		if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]) && func_41(unk_0xC1A5EC5C986B98AD(), uLocal_116[0 /*4*/], 1) >= 250f)
		{
			flag = true;
		
			for (i = 0; i <= 10; i = i + 1)
			{
				if (!func_39(i, true))
					flag = false;
			}
		
			if (flag)
				func_37();
		}
	}

	if (unk_0x7DE17ACDD8BA2642(uLocal_116[0 /*4*/]) && !_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_116[0 /*4*/]))
	{
		flag = true;
	
		for (i = 0; i <= 10; i = i + 1)
		{
			if (!func_39(i, true))
				flag = false;
		}
	
		if (flag)
			func_37();
	}

	return;
}

BOOL func_107() // Position - 0x5874
{
	if (!_CAN_ENTER_FREEROAM_STATE(5))
		return true;

	if (func_116())
		return true;

	if (!unk_0x66599E73DBA5A850(unk_0xC1A5EC5C986B98AD()))
		if (SYSTEM::VMAG2(unk_0x7A2C98D06C9C1956(unk_0xC1A5EC5C986B98AD())) > 1369f && !func_115())
			return false;

	if (func_108(100f, true) != -1)
		return true;

	return false;
}

int func_108(float fParam0, BOOL bParam1) // Position - 0x58D6
{
	var unk;
	int num;
	int num2;
	float num3;
	float num4;
	int num5;
	int i;
	BOOL flag;

	num2 = -1;
	num3 = fParam0;

	if (unk_0x48DC6A21D1CAD89C(unk_0x93E99A2DBCBA9CFA()))
	{
		if (func_23(_GET_CURRENT_PLAYER_CHARACTER()))
		{
			num5 = func_20();
			i = 0;
		
			for (i = 0; i < 63; i = i + 1)
			{
				num = i;
			
				if (IS_BIT_SET(Global_113648.f_18576[num /*6*/], 2) && !IS_BIT_SET(Global_113648.f_18576[num /*6*/], 3))
				{
					func_109(num, &unk);
					num4 = unk_0x2E496FE654DA4166(unk_0x30BE8A934C020461(unk_0xC1A5EC5C986B98AD(), 0), unk.f_6, 1);
				
					if (num4 < num3)
					{
						flag = true;
					
						if (bParam1)
							if (num5 != unk.f_26)
								flag = false;
					
						if (flag)
						{
							num2 = num;
							num3 = num4;
						}
					}
				}
			}
		}
	}

	return num2;
}

void func_109(int iParam0, var uParam1) // Position - 0x5987
{
	switch (iParam0)
	{
		case 0:
			func_110(uParam1, "Abigail1", func_112(iParam0), 0, 0, 4, -1604.668f, 5239.1f, 3.01f, 66, "", 109, 0, "ambient_Diving", 0, 0, 1, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 1:
			func_110(uParam1, "Abigail2", func_112(iParam0), 0, 0, 4, -1592.84f, 5214.04f, 3.01f, 400, "", 110, 0, "", 0, 0, -1, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 2:
			func_110(uParam1, "Barry1", func_112(iParam0), 0, 1, 4, 190.26f, -956.35f, 29.63f, 381, "", 74, 0, "", 0, 1, -1, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 3:
			func_110(uParam1, "Barry2", func_112(iParam0), 0, 1, 4, 190.26f, -956.35f, 29.63f, 381, "", -1, 0, "", 0, 1, -1, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 4:
			func_110(uParam1, "Barry3", func_112(iParam0), 0, 1, 4, 414f, -761f, 29f, 381, "", -1, 0, "", 164, 1, -1, 0, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 5:
			func_110(uParam1, "Barry3A", func_112(iParam0), 1, 1, 0, 1199.27f, -1255.63f, 34.23f, 381, "BARSTASH" /*Areas where you can find vehicles with a hidden stash have been marked on the map. Collect these vehicles for Barry.*/, 84, 0, "", 166, 0, 7, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 6:
			func_110(uParam1, "Barry3C", func_112(iParam0), 3, 1, 0, -468.9f, -1713.06f, 18.21f, 381, "", 84, 0, "", 166, 0, 7, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 7:
			func_110(uParam1, "Barry4", func_112(iParam0), 0, 1, 4, 237.65f, -385.41f, 44.4f, 381, "", 85, 0, "postRC_Barry4", 0, 0, -1, 4, 2, 800, 2000, func_111(iParam0), 0, 0);
			break;
	
		case 8:
			func_110(uParam1, "Dreyfuss1", func_112(iParam0), 0, 2, 4, -1458.97f, 485.99f, 115.38f, 66, "LETTERS_HINT" /*The killer's identity and location have been revealed.*/, 106, 0, "", 0, 0, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 9:
			func_110(uParam1, "Epsilon1", func_112(iParam0), 0, 3, 4, -1622.89f, 4204.87f, 83.3f, 66, "", 86, 0, "", 0, 1, 10, 4, 1, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 10:
			func_110(uParam1, "Epsilon2", func_112(iParam0), 0, 3, 4, 242.7f, 362.7f, 104.74f, 206, "", 87, 16, "", 0, 0, 11, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 11:
			func_110(uParam1, "Epsilon3", func_112(iParam0), 0, 3, 4, 1835.53f, 4705.86f, 38.1f, 206, "", 88, 16, "epsCars", 0, 0, 12, 4, 1, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 12:
			func_110(uParam1, "Epsilon4", func_112(iParam0), 0, 3, 4, 1826.13f, 4698.88f, 38.92f, 206, "", 90, 16, "postRC_Epsilon4", 0, 0, 13, 4, 1, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 13:
			func_110(uParam1, "Epsilon5", func_112(iParam0), 0, 3, 4, 637.02f, 119.7093f, 89.5f, 206, "", 89, 16, "epsRobes", 0, 0, 14, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 14:
			func_110(uParam1, "Epsilon6", func_112(iParam0), 0, 3, 4, -2892.93f, 3192.37f, 11.66f, 206, "", 93, 0, "", 0, 0, 15, 4, 1, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 15:
			func_110(uParam1, "Epsilon7", func_112(iParam0), 0, 3, 4, 524.43f, 3079.82f, 39.48f, 206, "", -1, 16, "epsDesert", 0, 0, 16, 4, 1, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 16:
			func_110(uParam1, "Epsilon8", func_112(iParam0), 0, 3, 4, -697.75f, 45.38f, 43.03f, 206, "", 94, 16, "epsilonTract", 0, 0, -1, 4, 1, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 17:
			func_110(uParam1, "Extreme1", func_112(iParam0), 0, 4, 4, -188.22f, 1296.1f, 302.86f, 66, "", -1, 0, "", 4, 1, 18, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 18:
			func_110(uParam1, "Extreme2", func_112(iParam0), 0, 4, 4, -954.19f, -2760.05f, 14.64f, 382, "", 96, 0, "", 171, 0, 19, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 19:
			func_110(uParam1, "Extreme3", func_112(iParam0), 0, 4, 4, -63.8f, -809.5f, 321.8f, 382, "", 97, 0, "", 0, 0, 20, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 20:
			func_110(uParam1, "Extreme4", func_112(iParam0), 0, 4, 4, 1731.41f, 96.96f, 170.39f, 382, "", 98, 16, "", 0, 0, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 21:
			func_110(uParam1, "Fanatic1", func_112(iParam0), 0, 5, 4, -1877.82f, -440.649f, 45.05f, 405, "", 74, 0, "", 0, 1, -1, 4, 1, 700, 2000, func_111(iParam0), 1, 0);
			break;
	
		case 22:
			func_110(uParam1, "Fanatic2", func_112(iParam0), 0, 5, 4, 809.66f, 1279.76f, 360.49f, 405, "", -1, 0, "", 0, 1, -1, 4, 4, 700, 2000, func_111(iParam0), 1, 0);
			break;
	
		case 23:
			func_110(uParam1, "Fanatic3", func_112(iParam0), 0, 5, 4, -915.6f, 6139.2f, 5.5f, 405, "", -1, 0, "", 0, 1, -1, 4, 2, 700, 2000, func_111(iParam0), 0, 1);
			break;
	
		case 24:
			func_110(uParam1, "Hao1", func_112(iParam0), 0, 6, 4, -72.29f, -1260.63f, 28.14f, 66, "", -1, 0, "controller_Races", 13, 1, -1, 4, 2, 2000, 500, func_111(iParam0), 0, 1);
			break;
	
		case 25:
			func_110(uParam1, "Hunting1", func_112(iParam0), 0, 7, 4, 1804.32f, 3931.33f, 32.82f, 66, "", -1, 0, "", 174, 1, 26, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 26:
			func_110(uParam1, "Hunting2", func_112(iParam0), 0, 7, 4, -684.17f, 5839.16f, 16.09f, 384, "", 99, 0, "", 7, 0, -1, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 27:
			func_110(uParam1, "Josh1", func_112(iParam0), 0, 8, 4, -1104.93f, 291.25f, 64.3f, 66, "", -1, 0, "forSaleSigns", 0, 1, 28, 4, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 28:
			func_110(uParam1, "Josh2", func_112(iParam0), 0, 8, 4, 565.39f, -1772.88f, 29.77f, 385, "", 105, 0, "", 0, 0, 29, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 29:
			func_110(uParam1, "Josh3", func_112(iParam0), 0, 8, 4, 565.39f, -1772.88f, 29.77f, 385, "", -1, 16, "", 0, 0, 30, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 30:
			func_110(uParam1, "Josh4", func_112(iParam0), 0, 8, 4, -1104.93f, 291.25f, 64.3f, 385, "", -1, 36, "", 0, 0, -1, 4, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 31:
			func_110(uParam1, "Maude1", func_112(iParam0), 0, 9, 4, 2726.1f, 4145f, 44.3f, 66, "", -1, 0, "BailBond_Launcher", 0, 1, -1, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 32:
			func_110(uParam1, "Minute1", func_112(iParam0), 0, 10, 4, 327.85f, 3405.7f, 35.73f, 66, "", -1, 0, "", 0, 1, 33, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 33:
			func_110(uParam1, "Minute2", func_112(iParam0), 0, 10, 4, 18f, 4527f, 105f, 386, "", -1, 10, "", 0, 0, 34, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 34:
			func_110(uParam1, "Minute3", func_112(iParam0), 0, 10, 4, -303.82f, 6211.29f, 31.05f, 386, "", -1, 10, "", 0, 0, -1, 4, 4, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 35:
			func_110(uParam1, "MrsPhilips1", func_112(iParam0), 0, 11, 4, 1972.59f, 3816.43f, 32.42f, 66, "", -1, 0, "ambient_MrsPhilips", 0, 1, -1, 4, 4, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 36:
			func_110(uParam1, "MrsPhilips2", func_112(iParam0), 0, 11, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 0, 1, -1, 4, 4, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 37:
			func_110(uParam1, "Nigel1", func_112(iParam0), 0, 12, 4, -1097.16f, 790.01f, 164.52f, 66, "", -1, 0, "", 177, 1, -1, 1, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 38:
			func_110(uParam1, "Nigel1A", func_112(iParam0), 0, 12, 1, -558.65f, 284.49f, 90.86f, 149, "NIGITEMS" /*Areas where you can find celebrity items have been marked on the map. Steal these items for Nigel and Mrs. Thornhill.*/, 100, 0, "", 0, 0, 42, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 39:
			func_110(uParam1, "Nigel1B", func_112(iParam0), 0, 12, 1, -1034.15f, 366.08f, 80.11f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_111(iParam0), 1, 1);
			break;
	
		case 40:
			func_110(uParam1, "Nigel1C", func_112(iParam0), 0, 12, 1, -623.91f, -266.17f, 37.76f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_111(iParam0), 1, 1);
			break;
	
		case 41:
			func_110(uParam1, "Nigel1D", func_112(iParam0), 0, 12, 1, -1096.85f, 67.68f, 52.95f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_111(iParam0), 1, 1);
			break;
	
		case 42:
			func_110(uParam1, "Nigel2", func_112(iParam0), 0, 12, 4, -1310.7f, -640.22f, 26.54f, 149, "", -1, 8, "", 0, 0, 43, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 43:
			func_110(uParam1, "Nigel3", func_112(iParam0), 0, 12, 4, -44.75f, -1288.67f, 28.21f, 149, "", -1, 16, "postRC_Nigel3", 0, 0, -1, 4, 4, 0, 2359, func_111(iParam0), 1, 1);
			break;
	
		case 44:
			func_110(uParam1, "Omega1", func_112(iParam0), 0, 13, 4, 2468.51f, 3437.39f, 49.9f, 66, "", -1, 0, "spaceshipParts", 0, 1, 45, 4, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 45:
			func_110(uParam1, "Omega2", func_112(iParam0), 0, 13, 4, 2319.44f, 2583.58f, 46.76f, 387, "", 107, 0, "", 0, 0, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 46:
			func_110(uParam1, "Paparazzo1", func_112(iParam0), 0, 14, 4, -149.75f, 285.81f, 93.67f, 66, "", -1, 0, "", 0, 1, 47, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 47:
			func_110(uParam1, "Paparazzo2", func_112(iParam0), 0, 14, 4, -70.71f, 301.43f, 106.79f, 389, "", -1, 8, "", 0, 0, 48, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 48:
			func_110(uParam1, "Paparazzo3", func_112(iParam0), 0, 14, 4, -257.22f, 292.85f, 90.63f, 389, "", -1, 8, "", 183, 1, -1, 2, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 49:
			func_110(uParam1, "Paparazzo3A", func_112(iParam0), 0, 14, 2, 305.52f, 157.19f, 102.94f, 389, "PAPPHOTO" /*Areas where you can find celebrity photo opportunities have been marked on the map. Track down and photograph these celebrities for Beverly.*/, 102, 0, "", 0, 0, 51, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 50:
			func_110(uParam1, "Paparazzo3B", func_112(iParam0), 0, 14, 2, 1040.96f, -534.42f, 60.17f, 389, "", 102, 0, "", 0, 0, 51, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 51:
			func_110(uParam1, "Paparazzo4", func_112(iParam0), 0, 14, 4, -484.2f, 229.68f, 82.21f, 389, "", -1, 8, "", 0, 1, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 52:
			func_110(uParam1, "Rampage1", func_112(iParam0), 0, 15, 4, 908f, 3643.7f, 32.2f, 66, "", -1, 0, "", 0, 1, 54, 4, 4, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 54:
			func_110(uParam1, "Rampage3", func_112(iParam0), 0, 15, 4, 465.1f, -1849.3f, 27.8f, 84, "", -1, 0, "", 0, 1, 55, 4, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 55:
			func_110(uParam1, "Rampage4", func_112(iParam0), 0, 15, 4, -161f, -1669.7f, 33f, 84, "", -1, 0, "", 0, 0, 56, 4, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 56:
			func_110(uParam1, "Rampage5", func_112(iParam0), 0, 15, 4, -1298.2f, 2504.14f, 21.09f, 84, "", -1, 0, "", 0, 0, 53, 4, 4, 0, 2359, func_111(iParam0), 0, 0);
			break;
	
		case 53:
			func_110(uParam1, "Rampage2", func_112(iParam0), 0, 15, 4, 1181.5f, -400.1f, 67.5f, 84, "", -1, 0, "rampage_controller", 0, 0, -1, 4, 4, 0, 2359, func_111(iParam0), 1, 0);
			break;
	
		case 57:
			func_110(uParam1, "TheLastOne", func_112(iParam0), 0, 16, 4, -1298.98f, 4640.16f, 105.67f, 66, "", 133, 1, "", 0, 1, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 58:
			func_110(uParam1, "Tonya1", func_112(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 66, "AM_H_RCFS" /*Strangers and Freaks can be found throughout San Andreas at ~HUD_COLOUR_FRANKLIN~~BLIP_RANDOM_CHARACTER~~s~*/, -1, 0, "ambient_TonyaCall", 24, 1, 59, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 59:
			func_110(uParam1, "Tonya2", func_112(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 388, "", -1, 48, "ambient_Tonya", 185, 0, 60, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 60:
			func_110(uParam1, "Tonya3", func_112(iParam0), 0, 17, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 187, 0, 61, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 61:
			func_110(uParam1, "Tonya4", func_112(iParam0), 0, 17, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 0, 0, 62, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		case 62:
			func_110(uParam1, "Tonya5", func_112(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 388, "", -1, 48, "", 0, 0, -1, 4, 2, 0, 2359, func_111(iParam0), 0, 1);
			break;
	
		default:
			break;
	}

	return;
}

void func_110(var uParam0, char* sParam1, var uParam2, var uParam3, int iParam4, int iParam5, int iParam6, float fParam7, float fParam8, float fParam9, int iParam10, char* sParam11, int iParam12, int iParam13, char* sParam14, int iParam15, int iParam16, int iParam17, int iParam18, int iParam19, int iParam20, int iParam21, int iParam22, int iParam23, int iParam24) // Position - 0x6B31
{
	uParam0->f_4 = iParam5;
	*uParam0 = sParam1;
	uParam0->f_1 = { uParam2 };
	uParam0->f_3 = iParam4;
	uParam0->f_5 = iParam6;
	uParam0->f_6 = { fParam7 };
	uParam0->f_9 = iParam10;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->f_10), sParam11, 16);
	uParam0->f_14 = iParam12;
	uParam0->f_15 = iParam13;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->f_16), sParam14, 24);
	uParam0->f_22 = iParam15;
	uParam0->f_23 = iParam16;
	uParam0->f_24 = iParam17;
	uParam0->f_25 = iParam18;
	uParam0->f_26 = iParam19;
	uParam0->f_27 = iParam20;
	uParam0->f_28 = iParam21;
	uParam0->f_29 = iParam22;
	uParam0->f_30 = iParam23;
	uParam0->f_31 = iParam24;
	return;
}

int func_111(int iParam0) // Position - 0x6BC2
{
	switch (iParam0)
	{
		case 0:
			return 0;
	
		case 1:
			return 0;
	
		case 2:
			return 1;
	
		case 3:
			return 1;
	
		case 4:
			return 0;
	
		case 5:
			return 1;
	
		case 6:
			return 1;
	
		case 7:
			return 0;
	
		case 8:
			return 1;
	
		case 9:
			return 0;
	
		case 10:
			return 0;
	
		case 11:
			return 0;
	
		case 12:
			return 1;
	
		case 13:
			return 0;
	
		case 14:
			return 1;
	
		case 15:
			return 0;
	
		case 16:
			return 1;
	
		case 17:
			return 1;
	
		case 18:
			return 1;
	
		case 19:
			return 1;
	
		case 20:
			return 1;
	
		case 21:
			return 1;
	
		case 22:
			return 1;
	
		case 23:
			return 1;
	
		case 24:
			return 1;
	
		case 25:
			return 1;
	
		case 26:
			return 1;
	
		case 27:
			return 0;
	
		case 28:
			return 1;
	
		case 29:
			return 1;
	
		case 30:
			return 1;
	
		case 31:
			return 0;
	
		case 32:
			return 1;
	
		case 33:
			return 1;
	
		case 34:
			return 1;
	
		case 35:
			return 0;
	
		case 36:
			return 0;
	
		case 37:
			return 0;
	
		case 38:
			return 1;
	
		case 39:
			return 1;
	
		case 40:
			return 1;
	
		case 41:
			return 1;
	
		case 42:
			return 1;
	
		case 43:
			return 1;
	
		case 44:
			return 0;
	
		case 45:
			return 0;
	
		case 46:
			return 1;
	
		case 47:
			return 1;
	
		case 48:
			return 0;
	
		case 49:
			return 1;
	
		case 50:
			return 1;
	
		case 51:
			return 1;
	
		case 52:
			return 1;
	
		case 54:
			return 1;
	
		case 55:
			return 1;
	
		case 56:
			return 1;
	
		case 53:
			return 1;
	
		case 57:
			return 1;
	
		case 58:
			return 1;
	
		case 59:
			return 1;
	
		case 60:
			return 1;
	
		case 61:
			return 1;
	
		case 62:
			return 1;
	
		default:
			break;
	}

	return 0;
}

struct<2> func_112(int iParam0) // Position - 0x6F08
{
	var unk;
	var unk3;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 8);
	unk3 = { func_113(iParam0) };

	if (unk_0x2AC37494BBF1DB16(&unk3))
	{
	}
	else
	{
		TEXT_LABEL_ASSIGN_STRING(&unk, "RC_", 8);
		TEXT_LABEL_APPEND_STRING(&unk, &unk3, 8);
	}

	return unk;
}

struct<2> func_113(int iParam0) // Position - 0x6F3F
{
	var unk;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 8);

	switch (iParam0)
	{
		case 0:
			TEXT_LABEL_ASSIGN_STRING(&unk, "ABI1", 8);
			break;
	
		case 1:
			TEXT_LABEL_ASSIGN_STRING(&unk, "ABI2", 8);
			break;
	
		case 2:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA1", 8);
			break;
	
		case 3:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA2", 8);
			break;
	
		case 4:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3", 8);
			break;
	
		case 5:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3A", 8);
			break;
	
		case 6:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3C", 8);
			break;
	
		case 7:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA4", 8);
			break;
	
		case 8:
			TEXT_LABEL_ASSIGN_STRING(&unk, "DRE1", 8);
			break;
	
		case 9:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS1", 8);
			break;
	
		case 10:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS2", 8);
			break;
	
		case 11:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS3", 8);
			break;
	
		case 12:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS4", 8);
			break;
	
		case 13:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS5", 8);
			break;
	
		case 14:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS6", 8);
			break;
	
		case 15:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS7", 8);
			break;
	
		case 16:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS8", 8);
			break;
	
		case 17:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT1", 8);
			break;
	
		case 18:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT2", 8);
			break;
	
		case 19:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT3", 8);
			break;
	
		case 20:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT4", 8);
			break;
	
		case 21:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN1", 8);
			break;
	
		case 22:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN2", 8);
			break;
	
		case 23:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN3", 8);
			break;
	
		case 24:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HAO1", 8);
			break;
	
		case 25:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HUN1", 8);
			break;
	
		case 26:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HUN2", 8);
			break;
	
		case 27:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS1", 8);
			break;
	
		case 28:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS2", 8);
			break;
	
		case 29:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS3", 8);
			break;
	
		case 30:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS4", 8);
			break;
	
		case 31:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MAU1", 8);
			break;
	
		case 32:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN1", 8);
			break;
	
		case 33:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN2", 8);
			break;
	
		case 34:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN3", 8);
			break;
	
		case 35:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MRS1", 8);
			break;
	
		case 36:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MRS2", 8);
			break;
	
		case 37:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1", 8);
			break;
	
		case 38:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1A", 8);
			break;
	
		case 39:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1B", 8);
			break;
	
		case 40:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1C", 8);
			break;
	
		case 41:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1D", 8);
			break;
	
		case 42:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI2", 8);
			break;
	
		case 43:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI3", 8);
			break;
	
		case 44:
			TEXT_LABEL_ASSIGN_STRING(&unk, "OME1", 8);
			break;
	
		case 45:
			TEXT_LABEL_ASSIGN_STRING(&unk, "OME2", 8);
			break;
	
		case 46:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA1", 8);
			break;
	
		case 47:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA2", 8);
			break;
	
		case 48:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3", 8);
			break;
	
		case 49:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3A", 8);
			break;
	
		case 50:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3B", 8);
			break;
	
		case 51:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA4", 8);
			break;
	
		case 52:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM1", 8);
			break;
	
		case 53:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM2", 8);
			break;
	
		case 54:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM3", 8);
			break;
	
		case 55:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM4", 8);
			break;
	
		case 56:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM5", 8);
			break;
	
		case 57:
			TEXT_LABEL_ASSIGN_STRING(&unk, "SAS1", 8);
			break;
	
		case 58:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON1", 8);
			break;
	
		case 59:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON2", 8);
			break;
	
		case 60:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON3", 8);
			break;
	
		case 61:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON4", 8);
			break;
	
		case 62:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON5", 8);
			break;
	
		default:
			break;
	}

	return unk;
}

eCharacter _GET_CURRENT_PLAYER_CHARACTER() // Position - 0x738B
{
	func_21();
	return Global_113648.f_2365.f_539.f_4321;
}

BOOL func_115() // Position - 0x73A4
{
	if (unk_0xCC17806DB0C41C19())
		if (unk_0x27654E358E874F45() == 1f)
			return true;

	return false;
}

BOOL func_116() // Position - 0x73C1
{
	if (func_119() && !func_115())
		return true;

	if (func_118() && func_117())
		return true;

	return false;
}

BOOL func_117() // Position - 0x73F3
{
	return Global_113366 > 0;
}

BOOL func_118() // Position - 0x7401
{
	if (Global_97603 != -1)
		return true;

	return false;
}

BOOL func_119() // Position - 0x7416
{
	if (Global_97603 != -1)
		return IS_BIT_SET(Global_91469[Global_97603 /*34*/].f_15, 20);

	return false;
}

BOOL func_120() // Position - 0x7439
{
	if (Global_113637 == func_30() && unk_0xA847A0F368810680() && Global_113638)
		return true;

	return false;
}

BOOL func_121(int iParam0) // Position - 0x7464
{
	return IS_BIT_SET(iLocal_149, iParam0);
}

void func_122(int iParam0) // Position - 0x7471
{
	if (iParam0 == -1)
		iParam0 = func_30();

	if (iParam0 == -1)
		return;

	func_124(iParam0);
	unk_0x1A59C174B4130991(0);
	unk_0xB1BC77E1EAD07BAE(1);
	Global_113634 = 0;
	func_123();
	return;
}

void func_123() // Position - 0x74A7
{
	if (!unk_0x66599E73DBA5A850(unk_0xC1A5EC5C986B98AD()))
	{
		if (unk_0x3C3D6D026CF7F51B(unk_0xC1A5EC5C986B98AD(), 0))
			unk_0x9C56520AE72AFDBF(unk_0x504B9BB48D41C264(unk_0x95DC39736F6748E3(unk_0xC1A5EC5C986B98AD(), 0)), 1);
	
		unk_0x598D91BBD045C1F3(unk_0xC1A5EC5C986B98AD(), 32, 0);
	}

	return;
}

void func_124(int iParam0) // Position - 0x74E4
{
	Global_113637 = iParam0;
	return;
}

int func_125(var uParam0, var uParam1, var uParam2, int iParam3, int iParam4, BOOL bParam5, BOOL bParam6) // Position - 0x74F2
{
	BOOL flag;
	Vector3 vector;
	eCharacter i;
	Vector3 vector2;
	int num;

	if (!Global_152234)
		return 0;

	if (iParam3 == -1)
		iParam3 = func_30();

	if (iParam3 == -1)
		return 0;

	if (iParam3 == 31 || iParam3 == 32)
		if (!_IS_EXCLUSIVE_CONTENT_UNLOCKED())
			return 0;

	uLocal_43 = { uParam0 };
	flag = false;

	if (!flag)
	{
		if (unk_0x48DC6A21D1CAD89C(unk_0x93E99A2DBCBA9CFA()) && !unk_0x66599E73DBA5A850(unk_0xC1A5EC5C986B98AD()))
		{
			vector = { unk_0x30BE8A934C020461(unk_0xC1A5EC5C986B98AD(), 1) };
		
			if (SYSTEM::VMAG2(unk_0x7A2C98D06C9C1956(unk_0xC1A5EC5C986B98AD())) > 1369f && !func_115())
				return 0;
		}
	
		if (!Global_113648.f_9087)
			return 0;
	
		if (_IS_MISSION_REPEAT_ACTIVE(false))
			return 0;
	
		if (func_116())
			return 0;
	
		if (func_166())
			return 0;
	
		if (Global_113637 != -1)
			return 0;
	
		if (func_23(_GET_CURRENT_PLAYER_CHARACTER()))
			if (func_108(100f, true) != -1)
				return 0;
	
		if (unk_0x48DC6A21D1CAD89C(unk_0x93E99A2DBCBA9CFA()) && !bParam6)
			if (vector.f_2 - uLocal_43.f_2 > 50f)
				return 0;
	
		if (!func_165(iParam3))
			return 0;
	
		if (func_23(_GET_CURRENT_PLAYER_CHARACTER()))
			if (func_164(_GET_CURRENT_PLAYER_CHARACTER()) == 4 || func_164(_GET_CURRENT_PLAYER_CHARACTER()) == 5)
				return 0;
	
		if (func_23(_GET_CURRENT_PLAYER_CHARACTER()))
			if (!func_163(iParam3, iParam4, 145))
				return 0;
	
		if (!func_162(Global_113648.f_24997.f_43[iParam3]))
			return 0;
	
		if (unk_0xA5E11AF0A2B928C1() - Global_113639 < 150000)
			if (iParam3 != 30)
				return 0;
	
		if (func_160())
			return 0;
	
		if (unk_0x5ABCFD3441B37CA6())
			return 0;
	
		if (unk_0xA847A0F368810680())
			return 0;
	
		if (!func_150(4))
			return 0;
	
		if (!_CAN_ENTER_FREEROAM_STATE(5))
			return 0;
	
		if (func_149(iParam3, iParam4) && !bParam5)
			return 0;
	
		if (Global_3 && iParam3 != 10)
			return 0;
	
		if (unk_0x4D3D95146FD3490D(unk_0x41AD4BF315E01D41(unk_0xC1A5EC5C986B98AD())))
			if (unk_0x41AD4BF315E01D41(unk_0xC1A5EC5C986B98AD()) == unk_0x9CE0235348AE4A4D(377.153f, -717.567f, 10.0536f) || unk_0x41AD4BF315E01D41(unk_0xC1A5EC5C986B98AD()) == unk_0x9CE0235348AE4A4D(320.9934f, 265.2515f, 82.1221f) || unk_0x41AD4BF315E01D41(unk_0xC1A5EC5C986B98AD()) == unk_0x9CE0235348AE4A4D(-1425.5645f, -244.3f, 15.8053f))
				return 0;
	
		if (iParam3 == 9 && iParam4 == 2 || iParam4 == 5 && !func_149(0, 0))
			return 0;
	
		if (Global_32310)
			return 0;
	
		if (func_165(30) && !func_149(30, 0))
			if (iParam3 != 30)
				if (SYSTEM::VDIST2(vector, -61.2745f, -1100.4675f, 25.3752f) < 176400f)
					return 0;
	
		if (func_23(_GET_CURRENT_PLAYER_CHARACTER()))
		{
			for (i = CHAR_MICHAEL; i < CHAR_MULTIPLAYER; i = i + 1)
			{
				vector2 = { Global_113648.f_2365.f_539.f_2300[i /*3*/] };
				num = Global_113648.f_2365.f_539.f_2296[i];
			
				if (func_148(num))
					if (func_126(i))
						if (!func_63(vector2, 0f, 0f, 0f, false))
							if (SYSTEM::VDIST2(unk_0x30BE8A934C020461(unk_0xC1A5EC5C986B98AD(), 0), vector2) < 210f * 210f)
								if (_GET_CURRENT_PLAYER_CHARACTER() != i)
									return 0;
			}
		}
	}

	return 1;
}

BOOL func_126(eCharacter echParam0) // Position - 0x788C
{
	int num;

	num = Global_113648.f_2365.f_539.f_2296[echParam0];
	return func_127(num);
}

BOOL func_127(int iParam0) // Position - 0x78AD
{
	return func_128(iParam0, 1);
}

int func_128(int iParam0, int iParam1) // Position - 0x78BC
{
	var unk;
	var unk2;
	int num;
	int num2;
	int num3;
	int num4;

	if (!func_148(iParam0))
		return 0;

	func_129(iParam0, &unk, &unk2, &num, &num2, &num3, &num4);

	if (num4 > 0 || num3 > 0 || num2 > 0 || num >= iParam1)
		return 1;

	return 0;
}

void func_129(int iParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6) // Position - 0x790F
{
	func_130(func_141(), iParam0, uParam1, uParam2, uParam3, uParam4, uParam5, uParam6);
	return;
}

void func_130(int iParam0, int iParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7) // Position - 0x792D
{
	int num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;
	int num7;
	int num8;

	if (func_140(iParam0, iParam1))
	{
		num = func_139(iParam1);
		num2 = func_137(iParam0);
		num3 = func_137(iParam0) - func_137(iParam1);
		num4 = func_139(iParam0) - func_139(iParam1);
		num5 = func_136(iParam0) - func_136(iParam1);
		num6 = func_135(iParam0) - func_135(iParam1);
		num7 = func_134(iParam0) - func_134(iParam1);
		num8 = func_133(iParam0) - func_133(iParam1);
	}
	else
	{
		num = func_139(iParam0);
		num2 = func_137(iParam1);
		num3 = func_137(iParam1) - func_137(iParam0);
		num4 = func_139(iParam1) - func_139(iParam0);
		num5 = func_136(iParam1) - func_136(iParam0);
		num6 = func_135(iParam1) - func_135(iParam0);
		num7 = func_134(iParam1) - func_134(iParam0);
		num8 = func_133(iParam1) - func_133(iParam0);
	}

	while (num8 < 0)
	{
		num8 = num8 + 60;
		num7 = num7 - 1;
	}

	while (num8 > 59)
	{
		num8 = num8 - 60;
		num7 = num7 + 1;
	}

	while (num7 < 0)
	{
		num7 = num7 + 60;
		num6 = num6 - 1;
	}

	while (num7 > 59)
	{
		num7 = num7 - 60;
		num6 = num6 + 1;
	}

	while (num6 < 0)
	{
		num6 = num6 + 24;
		num5 = num5 - 1;
	}

	while (num6 > 23)
	{
		num6 = num6 - 24;
		num5 = num5 + 1;
	}

	while (num5 < 0)
	{
		while (num4 < 0)
		{
			num4 = num4 + 12;
			num3 = num3 - 1;
		}
	
		num5 = num5 + func_132(num, num2);
		num4 = num4 - 1;
		num = SYSTEM::ROUND(func_131(SYSTEM::TO_FLOAT(num + 1), 0f, 12f));
	}

	while (num4 < 0)
	{
		num4 = num4 + 12;
		num3 = num3 - 1;
	}

	while (num4 > 12)
	{
		num4 = num4 - 12;
		num3 = num3 + 1;
	}

	*uParam2 = num8;
	*uParam3 = num7;
	*uParam4 = num6;
	*uParam5 = num5;
	*uParam6 = num4;
	*uParam7 = num3;
	return;
}

float func_131(float fParam0, float fParam1, float fParam2) // Position - 0x7B2E
{
	float num;

	if (fParam1 == fParam2)
		return fParam1;

	num = fParam2 - fParam1;
	fParam0 = fParam0 - ((float)SYSTEM::ROUND((fParam0 - fParam1) / num) * num);

	if (fParam0 < fParam1)
		fParam0 = fParam0 + num;

	return fParam0;
}

int func_132(int iParam0, int iParam1) // Position - 0x7B70
{
	if (iParam1 < 0)
		iParam1 = 0;

	switch (iParam0)
	{
		case 0:
		case 2:
		case 4:
		case 6:
		case 7:
		case 9:
		case 11:
			return 31;
	
		case 3:
		case 5:
		case 8:
		case 10:
			return 30;
	
		case 1:
			if (iParam1 % 4 == 0)
				if (iParam1 % 100 != 0)
					return 29;
				else if (iParam1 % 400 == 0)
					return 29;
		
			return 28;
	}

	return 30;
}

int func_133(int iParam0) // Position - 0x7C12
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 20) & 63;
}

int func_134(int iParam0) // Position - 0x7C25
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 14) & 63;
}

int func_135(int iParam0) // Position - 0x7C38
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 9) & 31;
}

int func_136(int iParam0) // Position - 0x7C4B
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 4) & 31;
}

int func_137(int iParam0) // Position - 0x7C5D
{
	return (SYSTEM::SHIFT_RIGHT(iParam0, 26) & 31 * IS_BIT_SET(iParam0, 31) ? -1 : 1) + 2011;
}

var func_138(BOOL bParam0, var uParam1, var uParam2) // Position - 0x7C7F
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

int func_139(int iParam0) // Position - 0x7C96
{
	return iParam0 & 15;
}

BOOL func_140(int iParam0, int iParam1) // Position - 0x7CA3
{
	int num;
	int num2;

	if (!func_148(iParam1) || !func_148(iParam0))
		return 1;

	num = func_137(iParam0);
	num2 = func_137(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_139(iParam0);
	num2 = func_139(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_136(iParam0);
	num2 = func_136(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_135(iParam0);
	num2 = func_135(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_134(iParam0);
	num2 = func_134(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_133(iParam0);
	num2 = func_133(iParam1);

	if (num > num2)
		return 1;

	return 0;
}

int func_141() // Position - 0x7DAF
{
	var unk;

	func_147(&unk, unk_0x731F95B6458DCF80());
	func_146(&unk, unk_0x77BBAAED3E25322C());
	func_145(&unk, unk_0x30DFE1FFD2CC7420());
	func_144(&unk, unk_0x8C0F17CAC308A14B());
	func_143(&unk, unk_0x61117764C67882B7());
	func_142(&unk, unk_0x367F557725B53815());
	return unk;
}

void func_142(var uParam0, int iParam1) // Position - 0x7DF5
{
	if (iParam1 <= 0)
		return;

	if (iParam1 > 2043 || iParam1 < 1979)
		return;

	*uParam0 = *uParam0 - *uParam0 & 2080374784;

	if (iParam1 < 2011)
	{
		*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(2011 - iParam1, 26);
		*uParam0 = *uParam0 | -2147483648;
	}
	else
	{
		*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1 - 2011, 26);
		*uParam0 = *uParam0 - *uParam0 & -2147483648;
	}

	return;
}

void func_143(var uParam0, int iParam1) // Position - 0x7E7B
{
	if (iParam1 < 0 || iParam1 > 11)
		return;

	*uParam0 = *uParam0 - *uParam0 & 15;
	*uParam0 = *uParam0 || iParam1;
	return;
}

void func_144(var uParam0, int iParam1) // Position - 0x7EAE
{
	int num;
	int num2;

	num = func_139(*uParam0);
	num2 = func_137(*uParam0);

	if (iParam1 < 1 || iParam1 > func_132(num, num2))
		return;

	*uParam0 = *uParam0 - *uParam0 & 496;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 4);
	return;
}

void func_145(var uParam0, int iParam1) // Position - 0x7EFF
{
	if (iParam1 < 0 || iParam1 > 24)
		return;

	*uParam0 = *uParam0 - *uParam0 & 15872;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 9);
	return;
}

void func_146(var uParam0, int iParam1) // Position - 0x7F39
{
	if (iParam1 < 0 || iParam1 >= 60)
		return;

	*uParam0 = *uParam0 - *uParam0 & 1032192;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 14);
	return;
}

void func_147(var uParam0, int iParam1) // Position - 0x7F74
{
	if (iParam1 < 0 || iParam1 >= 60)
		return;

	*uParam0 = *uParam0 - *uParam0 & 66060288;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 20);
	return;
}

BOOL func_148(int iParam0) // Position - 0x7FB0
{
	int num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;

	if (iParam0 == -15)
		return false;

	num = func_133(iParam0);

	if (num < 0 || num >= 60)
		return false;

	num2 = func_134(iParam0);

	if (num2 < 0 || num2 >= 60)
		return false;

	num3 = func_135(iParam0);

	if (num3 < 0 || num3 > 23)
		return false;

	num4 = func_137(iParam0);

	if (num4 <= 0 || num4 > 2043 || num4 < 1979)
		return false;

	num5 = func_139(iParam0);

	if (num5 < 0 || num5 > 11)
		return false;

	num6 = func_136(iParam0);

	if (num6 < 1 || num6 > func_132(num5, num4))
		return false;

	return true;
}

BOOL func_149(int iParam0, int iParam1) // Position - 0x808D
{
	if (IS_BIT_SET(Global_113648.f_24997.f_8[iParam0], iParam1))
		return true;

	return false;
}

BOOL func_150(int iParam0) // Position - 0x80AD
{
	eCharacter character;

	if (unk_0x48DC6A21D1CAD89C(unk_0x93E99A2DBCBA9CFA()))
	{
		if (unk_0x7DE17ACDD8BA2642(unk_0xC1A5EC5C986B98AD()))
		{
			if (!unk_0x66599E73DBA5A850(unk_0xC1A5EC5C986B98AD()))
			{
				character = _GET_CURRENT_PLAYER_CHARACTER();
			
				if (!func_23(character))
					return false;
			
				switch (iParam0)
				{
					case 9:
					case 0:
						if (!unk_0xE2A83DCD3E79D8D3(unk_0x93E99A2DBCBA9CFA()) || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) || unk_0x5CD470B5BDDAC029(unk_0xC1A5EC5C986B98AD()) || unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || unk_0x5150E13C3DAAE892(unk_0x93E99A2DBCBA9CFA()) || unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || func_159() || Global_112695 || Global_32166 || func_158() || func_157(8, -1) || func_156() || func_155() || func_154() || func_153() || Global_113648.f_7690.f_919[character] == 5)
							return false;
						break;
				
					case 1:
						if (unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || func_159() || Global_32166 || func_158() || func_157(8, -1) || func_154() || func_156() || func_155() || func_153() || Global_113648.f_7690.f_919[character] == 5)
							return false;
						break;
				
					case 2:
						if (!unk_0xE2A83DCD3E79D8D3(unk_0x93E99A2DBCBA9CFA()) || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) || unk_0x5CD470B5BDDAC029(unk_0xC1A5EC5C986B98AD()) || unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || unk_0x5150E13C3DAAE892(unk_0x93E99A2DBCBA9CFA()) || unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || func_159() || Global_112695 || Global_32166 || func_158() || func_157(8, -1) || func_154() || func_156() || func_155() || func_153() || Global_113648.f_7690.f_919[character] == 5 || Global_43804 != -1)
							return false;
						break;
				
					case 3:
						if (unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || func_159() || Global_112695 || Global_32166 || func_158() || func_157(8, -1) || func_156() || func_155() || func_153() || Global_113648.f_7690.f_919[character] == 5)
							return false;
						break;
				
					case 4:
						if (func_159() || unk_0x83C5A442AAD15BAF(unk_0x93E99A2DBCBA9CFA()) > 0 || func_157(8, -1) || func_153() || func_152() || Global_113648.f_7690.f_919[character] == 5)
							return false;
						break;
				
					case 5:
						if (func_157(8, -1) || func_156() || func_155() || func_152() || func_151())
							return false;
					
						if (unk_0xB53553DC4AAC7D8A() && unk_0x728FB8B438DDCAB6() != 3 && unk_0x19B3485E7C9D734D() < 8)
							return false;
						break;
				
					case 6:
						if (unk_0x7DE17ACDD8BA2642(unk_0xC1A5EC5C986B98AD()))
							if (unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || unk_0x83C5A442AAD15BAF(unk_0x93E99A2DBCBA9CFA()) > 0 || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) || unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || unk_0x5150E13C3DAAE892(unk_0x93E99A2DBCBA9CFA()) || func_159() || Global_32166 || func_158() || func_157(8, -1) || func_155() || func_154() || func_153() || Global_113648.f_7690.f_919[character] == 5)
								return false;
						break;
				
					case 7:
						if (unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || !unk_0x1B82EF141CC65934(unk_0x93E99A2DBCBA9CFA()) || !unk_0xE2A83DCD3E79D8D3(unk_0x93E99A2DBCBA9CFA()) || !unk_0x35DF833D93BCC488() || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) || unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || func_159() || func_155() || Global_112695 || Global_32166 || func_158() || Global_44446 || func_157(8, -1) || func_154() || func_152() || func_153() || Global_113648.f_7690.f_919[character] == 5)
							return false;
						break;
				
					case 8:
						if (unk_0xB8C39D031C377E33(unk_0xC1A5EC5C986B98AD(), 0) || !unk_0x1B82EF141CC65934(unk_0x93E99A2DBCBA9CFA()) || !unk_0xE2A83DCD3E79D8D3(unk_0x93E99A2DBCBA9CFA()) || !unk_0x35DF833D93BCC488() || unk_0xF356C80F3E8D2812(unk_0x93E99A2DBCBA9CFA(), 0) || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) || unk_0x3C3D6D026CF7F51B(unk_0xC1A5EC5C986B98AD(), 1) || unk_0x48A4D45B3B4CEFFD(unk_0xC1A5EC5C986B98AD()) || unk_0x854821B2A72B2E98(unk_0xC1A5EC5C986B98AD()) || unk_0x279D0BCF8F708D74(unk_0xC1A5EC5C986B98AD()) || unk_0xF41A4983A3778F18(unk_0x93E99A2DBCBA9CFA(), 1) || unk_0x5150E13C3DAAE892(unk_0x93E99A2DBCBA9CFA()) || func_159() || Global_112695 || Global_32166 || func_158() || func_157(8, -1) || func_154() || func_152() || func_156() || func_155() || func_153())
							return false;
						break;
				}
			}
			else
			{
				return false;
			}
		}
		else
		{
			return false;
		}
	}
	else
	{
		return false;
	}

	return true;
}

BOOL func_151() // Position - 0x87CA
{
	return Global_100720.f_1;
}

BOOL func_152() // Position - 0x87D8
{
	if (Global_97603 != -1)
		return IS_BIT_SET(Global_91469[Global_97603 /*34*/].f_15, 13);

	return false;
}

BOOL func_153() // Position - 0x87FB
{
	if (unk_0x24B651D85CCE5EB4(joaat("player_timetable_scene")) > 0)
		return true;

	return false;
}

BOOL func_154() // Position - 0x8815
{
	if (Global_78819)
		return true;
	else if (Global_63356 && !Global_63362)
		return true;

	return false;
}

BOOL func_155() // Position - 0x883F
{
	return Global_100733.f_388 > 0;
}

BOOL func_156() // Position - 0x8850
{
	return Global_100733.f_387 > 0;
}

BOOL func_157(int iParam0, int iParam1) // Position - 0x8861
{
	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
				return Global_1653913.f_203[iParam1];
			break;
	}

	return IS_BIT_SET(Global_1653913.f_1048, iParam0);
}

BOOL func_158() // Position - 0x8899
{
	return Global_1575060;
}

BOOL func_159() // Position - 0x88A5
{
	if (!unk_0xA26A9A07F761D8F8())
		return Global_98159.f_44 == 1;

	return false;
}

BOOL func_160() // Position - 0x88C1
{
	_UPDATE_CURRENT_PLAYER_CHARACTER();

	if (Global_9058[Global_20383 /*2811*/][0 /*281*/].f_259 == 2)
		return true;

	return false;
}

void _UPDATE_CURRENT_PLAYER_CHARACTER() // Position - 0x88E9
{
	if (func_22(14))
	{
		if (!unk_0x055111B11E6624FD(unk_0xC1A5EC5C986B98AD(), 0))
			if (unk_0x504B9BB48D41C264(unk_0xC1A5EC5C986B98AD()) == Global_113648.f_28052[0 /*29*/])
				Global_20383 = CHAR_MICHAEL;
			else if (unk_0x504B9BB48D41C264(unk_0xC1A5EC5C986B98AD()) == Global_113648.f_28052[1 /*29*/])
				Global_20383 = CHAR_FRANKLIN;
			else if (unk_0x504B9BB48D41C264(unk_0xC1A5EC5C986B98AD()) == Global_113648.f_28052[2 /*29*/])
				Global_20383 = CHAR_TREVOR;
			else
				Global_20383 = CHAR_MICHAEL;
	}
	else
	{
		Global_20383 = _GET_CURRENT_PLAYER_CHARACTER();
	
		if (Global_20383 == _CHAR_NULL)
			Global_20383 = CHAR_MULTIPLAYER;
	
		if (Global_78558)
			Global_20383 = CHAR_MULTIPLAYER;
	
		if (Global_20383 > CHAR_MULTIPLAYER)
			Global_20383 = CHAR_MULTIPLAYER;
	}

	return;
}

BOOL func_162(int iParam0) // Position - 0x898B
{
	return func_140(func_141(), iParam0);
}

BOOL func_163(int iParam0, int iParam1, eCharacter echParam2) // Position - 0x899D
{
	BOOL num;
	eCharacter character;

	num = 0;
	character = _GET_CURRENT_PLAYER_CHARACTER();

	if (echParam2 != 145 && echParam2 == 0 || echParam2 == 1 || echParam2 == 2)
		character = echParam2;

	switch (iParam0)
	{
		case 18:
			if (character == CHAR_TREVOR)
				num = 1;
			break;
	
		case 19:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 20:
			if (character != CHAR_FRANKLIN)
				num = 1;
			break;
	
		case 28:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 13:
			if (character == CHAR_MICHAEL)
				if (iParam1 == 2)
					num = 0;
				else
					num = 1;
			else
				num = 1;
			break;
	
		case 22:
			if (iParam1 == 2 || character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 30:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		default:
			num = 1;
			break;
	}

	return num;
}

int func_164(eCharacter echParam0) // Position - 0x8A81
{
	if (!func_23(echParam0))
		return 7;

	return Global_113648.f_7690.f_919[echParam0];
}

BOOL func_165(int iParam0) // Position - 0x8AA5
{
	int num;
	BOOL flag;

	if (iParam0 == 31 || iParam0 == 32)
		if (!_IS_EXCLUSIVE_CONTENT_UNLOCKED())
			return false;

	num = iParam0;

	if (num < 31)
	{
		flag = IS_BIT_SET(Global_113648.f_24997, num);
	}
	else
	{
		num = num - 31;
		flag = IS_BIT_SET(Global_113648.f_24997.f_1, num);
	}

	return flag;
}

BOOL func_166() // Position - 0x8AFD
{
	var unk;

	if (Global_32315)
	{
		if (unk_0x3C3D6D026CF7F51B(unk_0xC1A5EC5C986B98AD(), 0))
		{
			unk = unk_0x95DC39736F6748E3(unk_0xC1A5EC5C986B98AD(), 0);
		
			if (unk_0xFBD273FDBCF0C5BD(unk, 0))
				if (!unk_0x66599E73DBA5A850(unk_0xA66E176E5772E965(unk, 0, 0)))
					return true;
		}
	}

	return false;
}

BOOL _IS_EXCLUSIVE_CONTENT_UNLOCKED() // Position - 0x8B41
{
	var unk;

	if (Global_152523 == 2)
		return true;
	else if (Global_152523 == 3)
		return false;

	if (unk_0x7FFBE14C34D75E19())
	{
		if (unk_0x8BD41D5945F5762B())
		{
			if (unk_0x001B3EAD68D731CE())
			{
				unk_0xDD7756E2742E0F6D(joaat("SP_UNLOCK_EXCLUS_CONTENT"), &unk, -1);
				unk_0xECDAB41968FF21A8(&unk, 2);
				unk_0xECDAB41968FF21A8(&unk, 4);
				unk_0xECDAB41968FF21A8(&unk, 6);
				unk_0xECDAB41968FF21A8(&Global_25, 2);
				unk_0xECDAB41968FF21A8(&Global_25, 4);
				unk_0xECDAB41968FF21A8(&Global_25, 6);
				unk_0x3CC35ACFFC9C730E(joaat("SP_UNLOCK_EXCLUS_CONTENT"), unk, 1);
			
				if (unk_0x7D33DF1E1089E1D1())
				{
					unk = unk_0x516080EA609481E1(866);
					unk_0xECDAB41968FF21A8(&unk, 0);
					unk_0x705949B096008718(unk);
				}
			
				return true;
			}
		}
	}

	if (unk_0x7D33DF1E1089E1D1())
		if (IS_BIT_SET(unk_0x516080EA609481E1(866), 0))
			return true;

	return false;
}

int func_168() // Position - 0x8BF9
{
	var unk;
	var unk4;

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0xC1A5EC5C986B98AD()))
	{
		unk = { unk_0x30BE8A934C020461(unk_0xC1A5EC5C986B98AD(), 1) };
		unk_0xDE209AFE421606C6(unk, &unk4, 0, 0);
	
		if (unk.f_1 < 2149.0417f && unk_0xA87ECD5E2653C4A9(unk_0xC1A5EC5C986B98AD()) || unk_0xDA70CA6A0D1738D3(unk_0xC1A5EC5C986B98AD()) && unk.f_2 - unk4 >= 7.5f && unk.f_2 - unk4 >= 3f)
			return 0;
	
		if (unk.f_2 >= 100f && unk_0xA87ECD5E2653C4A9(unk_0xC1A5EC5C986B98AD()))
			return 0;
	}

	return 1;
}

void func_169(BOOL bParam0, BOOL bParam1) // Position - 0x8C91
{
	func_198();
	func_197();
	_DEBUG_PRINT(0);
	func_195(true, true, true, false);
	func_193(false);
	func_191();
	unk_0x8E09E8C52602EBB2(3054.991f, 1995.8871f, -20f, 3140.8171f, 2225.639f, 10.453f, 1, 1);
	unk_0xA2EC5ACB2B5D92A4();
	unk_0x61F7D56FFAFBF706(1);
	unk_0x4D66B8BC440E4F3E(0);
	func_190();
	func_189();
	func_188();
	func_187(bParam1);
	func_186(bParam1);
	unk_0x28F88BFAD6397895(uLocal_171);
	unk_0xFF2AF80F9EC7704C(1f);
	func_184();
	func_182();
	func_181(&uLocal_174, 1);
	func_181(&uLocal_174, 3);
	func_181(&uLocal_174, 4);
	func_181(&uLocal_174, 5);
	unk_0xA9F0F58A455A58AE(0);
	unk_0x9E2404A25985615A(0, 0, 3000, 1, 0, 0);
	func_70(false);

	if (bParam0)
	{
		func_170(-1);
		unk_0x675D9C12C73D3DE7();
	}

	return;
}

void func_170(int iParam0) // Position - 0x8D54
{
	var unk;

	if (iParam0 == -1)
		iParam0 = func_30();

	if (iParam0 == -1)
		return;

	if (func_120())
	{
		func_174(iParam0);
		unk_0x97DBA2AA6C70AFC7(0, 0);
		Global_113639 = unk_0xA5E11AF0A2B928C1();
		func_173(30000);
		TEXT_LABEL_ASSIGN_STRING(&unk, func_172(Global_113637, true), 64);
	
		if (func_29(Global_113637) > 0)
		{
			TEXT_LABEL_APPEND_STRING(&unk, " Variation ", 64);
			TEXT_LABEL_APPEND_INT(&unk, Global_113636, 64);
		}
	
		unk_0x36DDD3AC77856423(&unk, Global_113634, unk_0xA5E11AF0A2B928C1() - Global_113635, 0);
	}
	else if (IS_BIT_SET(Global_113644, 0) && Global_113648.f_24997.f_2 < 3)
	{
		unk_0x061B1200C95204CB(&Global_113644, 0);
	}

	func_171(&Global_32223);
	Global_113638 = false;
	func_124(-1);
	return;
}

void func_171(var uParam0) // Position - 0x8E06
{
	if (*uParam0 == -1)
		return;

	if (!*uParam0 == Global_43219)
	{
		*uParam0 = -1;
		return;
	}

	*uParam0 = -1;
	Global_43218 = 0;
	Global_43220 = 0;
	Global_43257 = 15;
	Global_63359 = 0;
	Global_63360 = 0;
	return;
}

char* func_172(int iParam0, BOOL bParam1) // Position - 0x8E43
{
	switch (iParam0)
	{
		case 0:
			return "RE_ACCIDENT";
	
		case 1:
			return "RE_ATMROBBERY";
	
		case 2:
			return "RE_BUSTOUR";
	
		case 3:
			return "RE_DOMESTIC";
	
		case 4:
			return "RE_GETAWAYDRIVER";
	
		case 5:
			return "RE_SHOPROBBERY";
	
		case 6:
			return "RE_SNATCHED";
	
		case 7:
			return "RE_LURED";
	
		case 8:
			return "RE_BIKETHIEFSTAMP";
	
		case 9:
			return "RE_SECURITYVAN";
	
		case 10:
			return "RE_PAPARAZZI";
	
		case 11:
			return "RE_CHASETHIEVES";
	
		case 12:
			return "RE_DEALGONEWRONG";
	
		case 13:
			return "RE_HITCHLIFT";
	
		case 14:
			return "RE_STAG";
	
		case 15:
			return "RE_ARREST";
	
		case 16:
			return "RE_CRASHRESCUE";
	
		case 17:
			return "RE_CARTHEFT";
	
		case 18:
			return "RE_CULTSHOOTOUT";
	
		case 19:
			return "RE_GANGFIGHT";
	
		case 20:
			return "RE_GANGINTIMIDATION";
	
		case 21:
			return "RE_PRISONVANBREAK";
	
		case 22:
			return "RE_PRISONERLIFT";
	
		case 23:
			return "RE_ABANDONEDCAR";
	
		case 24:
			return "RE_BURIAL";
	
		case 25:
			return "RE_MUGGING";
	
		case 26:
			return "RE_BIKETHIEF";
	
		case 27:
			return "RE_DRUNKDRIVER";
	
		case 28:
			return "RE_HOMELANDSECURITY";
	
		case 29:
			return "RE_BORDERPATROL";
	
		case 30:
			return "RE_SIMEONYETARIAN";
	
		case 31:
			return "RE_DUEL";
	
		case 32:
			return "RE_SEAPLANE";
	
		case 33:
			return "RE_MONKEYPHOTO";
	
		case -1:
			return "RE_NONE";
	}

	!bParam1;
	return "UNKNOWN";
}

void func_173(int iParam0) // Position - 0x908B
{
	Global_43808 = unk_0xA5E11AF0A2B928C1() + iParam0;
	return;
}

void func_174(int iParam0) // Position - 0x909D
{
	func_175(iParam0, 0, func_180(iParam0));
	return;
}

void func_175(int iParam0, int iParam1, int iParam2) // Position - 0x90B2
{
	int unk;
	var unk2;

	unk = func_141();
	func_178(&unk, 0, 0, iParam2, iParam1, 0, 0);
	func_177(iParam0, &unk);
	unk2 = { func_176(&unk) };
	return;
}

struct<16> func_176(var uParam0) // Position - 0x90E1
{
	var unk;
	int num;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 64);
	num = func_135(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, ":", 64);
	num = func_134(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, ":", 64);
	num = func_133(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "  ", 64);
	num = func_136(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "/", 64);
	num = func_139(*uParam0);

	if (num < 9)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num + 1, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "/", 64);
	TEXT_LABEL_APPEND_INT(&unk, func_137(*uParam0), 64);
	return unk;
}

void func_177(int iParam0, var uParam1) // Position - 0x91B1
{
	Global_113648.f_24997.f_43[iParam0] = *uParam1;
	return;
}

void func_178(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x91C9
{
	int num;
	int i;
	int num2;
	int j;
	int k;
	int l;
	int m;

	num = func_137(*uParam0);
	i = func_139(*uParam0);
	num2 = func_136(*uParam0);
	j = func_135(*uParam0);
	k = func_134(*uParam0);
	l = func_133(*uParam0);

	if (iParam6 == 0 && iParam5 == 0 && iParam4 == 0 && iParam3 == 0 && iParam2 == 0 && iParam1 == 0)
		return;

	if (iParam1 < 0)
		return;

	if (iParam2 < 0)
		return;

	if (iParam3 < 0)
		return;

	if (iParam4 < 0)
		return;

	if (iParam5 < 0)
		return;

	if (iParam6 < 0)
		return;

	for (l = l + iParam1; l >= 60; l = l - 60)
	{
		iParam2 = iParam2 + 1;
	}

	for (k = k + iParam2; k >= 60; k = k - 60)
	{
		iParam3 = iParam3 + 1;
	}

	for (j = j + iParam3; j >= 24; j = j - 24)
	{
		iParam4 = iParam4 + 1;
	}

	num2 = num2 + iParam4;

	for (m = func_132(i, num); num2 > m; m = func_132(i, num))
	{
		i = i + 1;
		num2 = num2 - m;
	
		if (i > 11)
		{
			num = num + 1;
			i = i - 12;
		}
	}

	for (i = i + iParam5; i > 11; i = i - 12)
	{
		iParam6 = iParam6 + 1;
	}

	num = num + iParam6;
	func_179(uParam0, l, k, j, num2, i, num);
	return;
}

void func_179(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x934B
{
	func_147(uParam0, iParam1);
	func_146(uParam0, iParam2);
	func_145(uParam0, iParam3);
	func_143(uParam0, iParam5);
	func_144(uParam0, iParam4);
	func_142(uParam0, iParam6);
	return;
}

int func_180(int iParam0) // Position - 0x9383
{
	int num;

	switch (iParam0)
	{
		case 23:
			num = 30;
			break;
	
		case 0:
			num = 30;
			break;
	
		case 15:
			num = 30;
			break;
	
		case 1:
			num = 200;
			break;
	
		case 26:
			num = 30;
			break;
	
		case 8:
			num = 30;
			break;
	
		case 29:
			num = 30;
			break;
	
		case 24:
			num = 30;
			break;
	
		case 2:
			num = 0;
			break;
	
		case 17:
			num = 30;
			break;
	
		case 11:
			num = 30;
			break;
	
		case 16:
			num = 30;
			break;
	
		case 18:
			num = 30;
			break;
	
		case 12:
			num = 120;
			break;
	
		case 3:
			num = 60;
			break;
	
		case 27:
			num = 60;
			break;
	
		case 19:
			num = 30;
			break;
	
		case 20:
			num = 30;
			break;
	
		case 4:
			num = 60;
			break;
	
		case 28:
			num = 30;
			break;
	
		case 13:
			num = 35;
			break;
	
		case 7:
			num = 30;
			break;
	
		case 25:
			num = 40;
			break;
	
		case 10:
			num = 30;
			break;
	
		case 22:
			num = 30;
			break;
	
		case 21:
			num = 30;
			break;
	
		case 5:
			num = 30;
			break;
	
		case 30:
			num = 60;
			break;
	
		case 9:
			num = 60;
			break;
	
		case 6:
			num = 40;
			break;
	
		case 14:
			num = 40;
			break;
	}

	return num;
}

void func_181(var uParam0, int iParam1) // Position - 0x9526
{
	if (uParam0->[iParam1 /*10*/].f_7 == 1)
		uParam0->[iParam1 /*10*/].f_7 = 0;

	return;
}

void func_182() // Position - 0x9543
{
	Global_20591 = 0;
	func_183();
	return;
}

void func_183() // Position - 0x9553
{
	unk_0x782234C275AFBA24();
	Global_22736 = 0;

	if (unk_0xE444FD7526C8BB46() || Global_20383.f_1 == 9 || Global_20382 == 1)
	{
		unk_0xD6625E11483B2324(0);
		Global_21725 = 6;
		Global_20383.f_1 = 3;
		return;
	}

	if (unk_0x0DCB385165FDE763())
	{
		unk_0xD6625E11483B2324(1);
		Global_21725 = 6;
		return;
	}

	return;
}

void func_184() // Position - 0x95AA
{
	Global_20591 = 0;
	func_185();
	return;
}

void func_185() // Position - 0x95BA
{
	unk_0x782234C275AFBA24();
	Global_22736 = 0;

	if (unk_0x0DCB385165FDE763())
	{
		unk_0xD6625E11483B2324(0);
		Global_21725 = 6;
	}

	return;
}

void func_186(BOOL bParam0) // Position - 0x95DB
{
	int i;

	for (i = 0; i < 5; i = i + 1)
	{
		func_92(i, bParam0);
	}

	return;
}

void func_187(BOOL bParam0) // Position - 0x95FD
{
	int i;

	for (i = 0; i < 11; i = i + 1)
	{
		func_40(i, bParam0);
	}

	return;
}

void func_188() // Position - 0x9620
{
	return;
}

void func_189() // Position - 0x9628
{
	unk_0x0DE8FEF68764819C(sLocal_137);
	unk_0x0DE8FEF68764819C(sLocal_138);
	return;
}

void func_190() // Position - 0x963C
{
	return;
}

void func_191() // Position - 0x9644
{
	int i;

	for (i = 0; i < 1; i = i + 1)
	{
		func_192(i);
	}

	return;
}

void func_192(int iParam0) // Position - 0x9664
{
	if (uLocal_172[iParam0] != 0)
	{
		unk_0x17D46728E18012D3(uLocal_172[iParam0], 0);
		uLocal_172[iParam0] = 0;
	}

	return;
}

void func_193(BOOL bParam0) // Position - 0x9688
{
	if (bParam0)
	{
		unk_0x8FE445B0989D8C71(unk_0x93E99A2DBCBA9CFA(), 0, 0);
		unk_0x508B7A54DEE39001(0);
		unk_0x6CDFCA6843DA90FF(unk_0x93E99A2DBCBA9CFA(), 0);
		unk_0xFF2AF80F9EC7704C(0f);
		unk_0xA11A4CE9E5914E55(unk_0x93E99A2DBCBA9CFA(), 0);
		unk_0x8234DA302DC655DC(0);
		unk_0x436EC806292A3DB8(3, 0);
		unk_0x436EC806292A3DB8(1, 0);
		unk_0x436EC806292A3DB8(8, 0);
		unk_0x436EC806292A3DB8(2, 0);
		unk_0x436EC806292A3DB8(6, 0);
		unk_0x436EC806292A3DB8(4, 0);
		unk_0x436EC806292A3DB8(12, 0);
		unk_0x436EC806292A3DB8(5, 0);
		func_194(9, true);
		func_194(8, true);
	}
	else
	{
		unk_0x508B7A54DEE39001(5);
		unk_0xFF2AF80F9EC7704C(1f);
		unk_0xA11A4CE9E5914E55(unk_0x93E99A2DBCBA9CFA(), 1);
		unk_0x8234DA302DC655DC(1);
		unk_0x436EC806292A3DB8(3, 1);
		unk_0x436EC806292A3DB8(1, 1);
		unk_0x436EC806292A3DB8(8, 1);
		unk_0x436EC806292A3DB8(2, 1);
		unk_0x436EC806292A3DB8(6, 1);
		unk_0x436EC806292A3DB8(4, 1);
		unk_0x436EC806292A3DB8(12, 1);
		unk_0x436EC806292A3DB8(5, 1);
		func_194(9, false);
		func_194(8, false);
	}

	return;
}

void func_194(int iParam0, BOOL bParam1) // Position - 0x975B
{
	if (bParam1)
		unk_0xECDAB41968FF21A8(&Global_32412, iParam0);
	else
		unk_0x061B1200C95204CB(&Global_32412, iParam0);

	return;
}

void func_195(BOOL bParam0, BOOL bParam1, BOOL bParam2, BOOL bParam3) // Position - 0x977D
{
	if (bParam0)
		func_182();

	if (bParam1)
	{
		if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS() || bParam0 && !bParam3)
			unk_0x8581263F2F024518();
	
		unk_0x1E408541295E76E9();
	}

	if (bParam2)
		unk_0xCD3C8E1022864F65(1);

	return;
}

BOOL _CONVERSATION_IS_DIALOGUE_IN_PROGRESS() // Position - 0x97BD
{
	if (Global_21725 != 0 || unk_0x0DCB385165FDE763())
		return true;

	return false;
}

void func_197() // Position - 0x97DF
{
	int i;

	for (i = 0; i < 11; i = i + 1)
	{
		func_43(&(uLocal_49[i /*6*/].f_1));
	}

	for (i = 0; i < 5; i = i + 1)
	{
		func_43(&(uLocal_116[i /*4*/].f_1));
	}

	return;
}

void func_198() // Position - 0x9824
{
	iLocal_47 = 0;
	iLocal_48 = 0;
	iLocal_148 = 0;
	iLocal_149 = 0;
	return;
}

BOOL _DOES_ENTITY_EXIST_AND_IS_ALIVE(int iParam0) // Position - 0x9838
{
	if (unk_0x7DE17ACDD8BA2642(iParam0))
		if (!unk_0x055111B11E6624FD(iParam0, 0))
			return true;

	return false;
}

