...
void main() // Position - 0x0
{
	...
	while (num == 1)
	{
		...
		if (func_84(pedLocal_240, PLAYER::PLAYER_PED_ID(), true) < 25f)
			if (!HUD::DOES_BLIP_EXIST(blLocal_248))
				func_80(&blLocal_248, &pedLocal_240, false);
			...
			case 8:
				...
				else if (bLocal_440 == true)
				{
					if (ENTITY::IS_ENTITY_PLAYING_ANIM(pedLocal_240, &uLocal_372, &uLocal_356, 3))
						func_52(Global_113593[iLocal_251], iLocal_297, ENTITY::GET_ENTITY_ANIM_CURRENT_TIME(pedLocal_240, &uLocal_372, &uLocal_356));
				}
				break;
	...
	while (true)
	{
		...
		if (ENTITY::IS_ENTITY_OCCLUDED(pedLocal_240) && func_84(pedLocal_240, PLAYER::PLAYER_PED_ID(), true) > 50f && !BRAIN::IS_WORLD_POINT_WITHIN_BRAIN_ACTIVATION_RANGE())
			func_113(1);
...
void func_1() // Position - 0x11FC
{
	...
	if (ENTITY::DOES_ENTITY_EXIST(pedLocal_448) && !ENTITY::IS_ENTITY_DEAD(pedLocal_448, false) && !ENTITY::IS_ENTITY_DEAD(pedLocal_240, false))
	{
		if (!bLocal_449)
		{
			if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(pedLocal_240, true), ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true), true) < 15f)
			{
				...
				TASK::TASK_FOLLOW_POINT_ROUTE(pedLocal_448, 1f, 0);
				bLocal_449 = true;
			}
		}
		else
		{
			PED::SET_IK_TARGET(pedLocal_448, 1, pedLocal_240, 0, 0f, 0f, 0f, 0, -1, -1);
		
			if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(pedLocal_448, true), 377.4565f, 154.6708f, 102.0838f, true) > 25f)
			{
				ENTITY::SET_PED_AS_NO_LONGER_NEEDED(&pedLocal_448);
				bLocal_450 = true;
			}
			else if (MISC::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(pedLocal_448, true), ENTITY::GET_ENTITY_COORDS(pedLocal_240, true), true) < 3f && iLocal_451 == 0)
			{
				iLocal_451 = 1;
			}
		}
...
{
	if (!ENTITY::IS_ENTITY_DEAD(pedLocal_240, false))
		AUDIO::REMOVE_ENTITY_FROM_AUDIO_MIX_GROUP(pedLocal_240, 0);
...
void func_104() // Position - 0x32B0
{
	...
	pedLocal_448 = PED::CREATE_PED(PED_TYPE_CIVMALE, joaat("A_M_M_Business_01"), 396.8498f, 143.3195f, 101.4659f, 360f, true, true);

	if (ENTITY::DOES_ENTITY_EXIST(pedLocal_448) && !ENTITY::IS_ENTITY_DEAD(pedLocal_448, false) && !ENTITY::IS_ENTITY_DEAD(pedLocal_240, false))
		PED::SET_IK_TARGET(pedLocal_448, 1, pedLocal_240, 0, 0f, 0f, 0f, 0, -1, -1);

	if (!ENTITY::IS_ENTITY_DEAD(pedLocal_240, false))
		AUDIO::ADD_ENTITY_TO_AUDIO_MIX_GROUP(pedLocal_240, "MAG_2_PAM_PED_GROUP", 0);
...
{
	hLocal_36 = joaat("U_F_O_MovieStar");
...
void func_113(int iParam0) // Position - 0x35DD
{
	...
	if (bLocal_241)
	{
		...
		if (MISC::ARE_STRINGS_EQUAL(&string1, &uLocal_502))
			func_13();
...
void func_117() // Position - 0x36FC
{
	...
	if (ENTITY::DOES_ENTITY_EXIST(pedLocal_448))
		ENTITY::SET_PED_AS_NO_LONGER_NEEDED(&pedLocal_448);
